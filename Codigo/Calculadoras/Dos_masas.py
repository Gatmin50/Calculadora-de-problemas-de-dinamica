import customtkinter as ctk
import tkinter as tk
from tkinter import ttk
import math
import turtle
import os

def programa_plano_inclinado_con_dos_cuerpos():

    # Crear la ventana de la calculadora
    ventana2 = ctk.CTk()
    ventana2.title("Calculadora de Dinámica")
    ventana2.configure(fg_color="black")

    os_name = os.name
    if os_name == 'nt':
        screen_width = ventana2.winfo_screenwidth()
        screen_height = ventana2.winfo_screenheight()
        x_offset = 0
        y_offset = 0
        ventana2.geometry(f"{screen_width}x{screen_height}+{x_offset}+{y_offset}")
        directorio = 'C:/calculadora_de_dinamica/'
        ruta_local = os.path.join(directorio, 'Plano_inclinado2.ico')
        ventana2.iconbitmap(ruta_local)
    elif os_name == 'posix':
        screen_width = ventana2.winfo_screenwidth()
        screen_height = ventana2.winfo_screenheight()
        ventana2.geometry(f"{screen_width}x{screen_height}")

    # Enunciado de la calculadora
    etiqueta_cu = ctk.CTkLabel(ventana2, text="Botones y cuadros de texto para la calculadora",text_color="white")
    etiqueta_cu.grid(column=2, row=0, columnspan=2, padx=10, pady=10)

    # Enunciado de la calculadora
    etiqueta_ca = ctk.CTkLabel(ventana2, text="Botones y cuadros de texto para el grafico",text_color="white")
    etiqueta_ca.grid(column=0, row=7, columnspan=2, padx=10, pady=10)

    # Crear las etiquetas y entradas de datos
    etiqueta_tension = ctk.CTkLabel(ventana2, text="Tension:",text_color="white")
    etiqueta_tension.grid(column=2, row=2, padx=5, pady=5)
    tension = ctk.CTkEntry(ventana2)
    tension.grid(column=3, row=2, padx=5, pady=5)
    
    etiqueta_friccion1 = ctk.CTkLabel(ventana2, text="Coeficiente de friccion 1:",text_color="white")
    etiqueta_friccion1.grid(column=0, row=3, padx=5, pady=5)
    friccion1 = ctk.CTkEntry(ventana2)
    friccion1.grid(column=1, row=3, padx=5, pady=5)
    
    etiqueta_friccion2 = ctk.CTkLabel(ventana2, text="Coeficiente de friccion 2:",text_color="white")
    etiqueta_friccion2.grid(column=2, row=3, padx=5, pady=5)
    friccion2 = ctk.CTkEntry(ventana2)
    friccion2.grid(column=3, row=3, padx=5, pady=5)

    etiqueta_aceleracion = ctk.CTkLabel(ventana2, text="Aceleracion:",text_color="white")
    etiqueta_aceleracion.grid(column=4, row=1, padx=5, pady=5)
    aceleracion = ctk.CTkEntry(ventana2)
    aceleracion.grid(column=5, row=1, padx=5, pady=5)

    etiqueta_masa1 = ctk.CTkLabel(ventana2, text="Masa 1 (Kg):",text_color="white")
    etiqueta_masa1.grid(column=0, row=1, padx=5, pady=5)

    masa1 = ctk.CTkEntry(ventana2)
    masa1.grid(column=1, row=1, padx=5, pady=5)

    etiqueta_gravedad = ctk.CTkLabel(ventana2, text="Gravedad:",text_color="white")
    etiqueta_gravedad.grid(column=0, row=2, padx=5, pady=5)

    Gravedad = ctk.CTkEntry(ventana2)
    Gravedad.grid(column=1, row=2, padx=5, pady=5)

    etiqueta_masa2 = ctk.CTkLabel(ventana2, text="Masa 2 (Kg):",text_color="white")
    etiqueta_masa2.grid(column=2, row=1, padx=5, pady=5)

    masa2 = ctk.CTkEntry(ventana2)
    masa2.grid(column=3, row=1, padx=5, pady=5)
    
    # Crear la etiqueta para mostrar el resultado
    label_resultado = ctk.CTkLabel(ventana2, text="" ,text_color="white")
    label_resultado.grid(column=4, row=2, columnspan=2, rowspan=2, padx=10, pady=10)

    # Crear la etiqueta para almacenar el resultado
    label_resultado1 = ctk.CTkLabel(ventana2, text="" ,text_color="white")
    label_resultado1.grid(column=7, row=1, columnspan=1, padx=10, pady=10)

    label_resultado2 = ctk.CTkLabel(ventana2, text="" ,text_color="white")
    label_resultado2.grid(column=7, row=2, columnspan=1, padx=10, pady=10)

    label_resultado3 = ctk.CTkLabel(ventana2, text="" ,text_color="white")
    label_resultado3.grid(column=7, row=3, columnspan=1, padx=10, pady=10)

    label_resultado4 = ctk.CTkLabel(ventana2, text="" ,text_color="white")
    label_resultado4.grid(column=7, row=4, columnspan=1, padx=10, pady=10)

    label_resultado5 = ctk.CTkLabel(ventana2, text="" ,text_color="white")
    label_resultado5.grid(column=7, row=5, columnspan=1, padx=10, pady=10)

    label_resultado6 = ctk.CTkLabel(ventana2, text="" ,text_color="white")
    label_resultado6.grid(column=7, row=6, columnspan=1, padx=10, pady=10)

    label_resultado7 = ctk.CTkLabel(ventana2, text="" ,text_color="white")
    label_resultado7.grid(column=7, row=7, columnspan=1, padx=10, pady=10)

    label_resultado8 = ctk.CTkLabel(ventana2, text="" ,text_color="white")
    label_resultado8.grid(column=7, row=8, columnspan=1, padx=10, pady=10)

    label_resultado9 = ctk.CTkLabel(ventana2, text="" ,text_color="white")
    label_resultado9.grid(column=7, row=9, columnspan=1, padx=10, pady=10)

    label_resultado10 = ctk.CTkLabel(ventana2, text="" ,text_color="white")
    label_resultado10.grid(column=7, row=10, columnspan=1, padx=10, pady=10)

    label_resultado11 = ctk.CTkLabel(ventana2, text="" ,text_color="white")
    label_resultado11.grid(column=7, row=11, columnspan=1, padx=10, pady=10)

    # Calculo para el Peso1
    def calcular_Peso1():

        try:
            m = float(masa1.get())
            g = float(Gravedad.get())
            x = m * g
            resultado = round(x, 3)
            label_resultado.config(text=f"El resultado es: {resultado}(N)")
            label_resultado1.config(text=f"El peso 1 es de {resultado}(N)")
        except ValueError:
            label_resultado.config(text=f"Todos los valores deben \n ser unicamente numéricos")

    # Calculo para el Peso2
    def calcular_Peso2():

        try:
            m = float(masa2.get())
            g = float(Gravedad.get())
            x = m * g
            resultado = round(x, 3)
            label_resultado.config(text=f"El resultado es: {resultado}(N)")
            label_resultado2.config(text=f"El peso 2 es de {resultado}(N)")
        except ValueError:
            label_resultado.config(text=f"Todos los valores deben \n ser unicamente numéricos")

    # Calculo para la Fricion 1
    def calcular_Friccion1():

        try:
            if angulo_de_inclinacion1.get() != "":
        
                m = float(masa1.get())
                g = float(Gravedad.get())
                mu = float(friccion1.get())
                cos = float(angulo_de_inclinacion1.get())
                x = mu * (m * g * math.cos(math.radians(cos)))
                resultado2 = round(x, 3)
                label_resultado.config(text=f"El resultado es: {resultado2}(N)")
                label_resultado3.config(text=f"La friccion 1 es de {resultado2}(N)")

            else:
                m = float(masa1.get())
                g = float(Gravedad.get())
                mu = float(friccion1.get())
                x = mu * (m * g)
                resultado2 = round(x, 3)
                label_resultado.config(text=f"El resultado es: {resultado2}(N)")
                label_resultado3.config(text=f"La friccion 1 es de {resultado2}(N)")
        except ValueError:
            label_resultado.config(text=f"Todos los valores deben \n ser unicamente numéricos")

    # Calculo para la Fricion 2
    def calcular_Friccion2():

        try:
            if angulo_de_inclinacion2.get() != "":
            
                m = float(masa2.get())
                g = float(Gravedad.get())
                mu = float(friccion2.get())
                cos = float(angulo_de_inclinacion2.get())
                x = mu * (m * g * math.cos(math.radians(cos)))
                resultado2 = round(x, 3)
                label_resultado.config(text=f"El resultado es: {resultado2}(N)")
                label_resultado4.config(text=f"La friccion 2 es de {resultado2}(N)")
        
            else:
                m = float(masa2.get())
                g = float(Gravedad.get())
                mu = float(friccion2.get())
                x = mu * (m * g)
                resultado2 = round(x, 3)
                label_resultado.config(text=f"El resultado es: {resultado2}(N)")
                label_resultado4.config(text=f"La friccion 2 es de {resultado2}(N)")
        except ValueError:
            label_resultado.config(text=f"Todos los valores deben \n ser unicamente numéricos")
    
    # Calculo para el Peso 1 en (Y)
    def calcular_Pesoy1():

        try:
            m = float(masa1.get())
            g = float(Gravedad.get())
            cos = float(angulo_de_inclinacion1.get())
            x = m * g * math.cos(math.radians(cos))
            resultado4 = round(x, 3)
            label_resultado.config(text=f"El resultado es: {resultado4}(N)")
            label_resultado5.config(text=f"El peso 1 en Y es: {resultado4}(N)")
        except ValueError:
            label_resultado.config(text=f"Todos los valores deben \n ser unicamente numéricos")
    
    # Calculo para el Peso 1 en (X)
    def calcular_Pesox1():

        try:
            m = float(masa1.get())
            g = float(Gravedad.get())
            sen = float(angulo_de_inclinacion1.get())
            x = m * g * math.sin(math.radians(sen))
            resultado4 = round(x, 3)
            label_resultado.config(text=f"El resultado es: {resultado4}(N)")
            label_resultado6.config(text=f"El peso 1 en X es: {resultado4}(N)")
        except ValueError:
            label_resultado.config(text=f"Todos los valores deben \n ser unicamente numéricos")
    
    # Calculo para el Peso 2 en (Y)
    def calcular_Pesoy2():

        try:
            m = float(masa2.get())
            g = float(Gravedad.get())
            cos = float(angulo_de_inclinacion2.get())
            x = m * g * math.cos(math.radians(cos))
            resultado4 = round(x, 3)
            label_resultado.config(text=f"El resultado es: {resultado4}(N)")
            label_resultado7.config(text=f"El peso 1 en Y es: {resultado4}(N)")
        except ValueError:
            label_resultado.config(text=f"Todos los valores deben \n ser unicamente numéricos")
    
    # Calculo para el Peso 2 en (X)
    def calcular_Pesox2():

        try:
            m = float(masa2.get())
            g = float(Gravedad.get())
            cos = float(angulo_de_inclinacion2.get())
            x = m * g * math.sin(math.radians(cos))
            resultado4 = round(x, 3)
            label_resultado.config(text=f"El resultado es: {resultado4}(N)")
            label_resultado8.config(text=f"El peso 1 en X es: {resultado4}(N)")
        except ValueError:
            label_resultado.config(text=f"Todos los valores deben \n ser unicamente numéricos")

    # Calculo para la aceleración1
    def calcular_aceleraciónfm1():

        try:
            m2 = float(masa2.get())
            m1 = float(masa1.get())
            g = float(Gravedad.get())
            cos1 = float(angulo_de_inclinacion1.get())
            cos2 = float(angulo_de_inclinacion2.get())
            mu1 = float(friccion1.get())
            mu2 = float(friccion2.get())
            fr1 = mu1 * (m1 * g * math.cos(math.radians(cos1)))
            fr2 = mu2 * (m2 * g * math.cos(math.radians(cos2)))
            px1 = m1 * g * math.cos(math.radians(cos1))
            px2 = m2 * g * math.cos(math.radians(cos2))
            z = -1
            a = z * (px2 - px1) - (fr1 - fr2)
            x = a / (m1 + m2)
            resultado4 = round(x, 3)
            if resultado4 >= 0:
                label_resultado.config(text=f"La aceleración es: {resultado4}(m/s²)")
                label_resultado9.config(text=f"La aceleración a favor \n de la masa1 es: {resultado4}(m/s²)")
            else:
                label_resultado.config(text=f"Este caso es imposible")
                label_resultado9.config(text=f"Este caso es imposible")
        except ValueError:
            label_resultado.config(text=f"Todos los valores deben \n ser unicamente numéricos")

    # Calculo para la aceleración2
    def calcular_aceleraciónfm2():

        try:
            m2 = float(masa2.get())
            m1 = float(masa1.get())
            g = float(Gravedad.get())
            cos1 = float(angulo_de_inclinacion1.get())
            cos2 = float(angulo_de_inclinacion2.get())
            mu1 = float(friccion1.get())
            mu2 = float(friccion2.get())
            fr1 = mu1 * (m1 * g * math.cos(math.radians(cos1)))
            fr2 = mu2 * (m2 * g * math.cos(math.radians(cos2)))
            px1 = m1 * g * math.cos(math.radians(cos1))
            px2 = m2 * g * math.cos(math.radians(cos2))
            z = 1
            a = z * (px2 - px1) - (fr1 - fr2)
            x = a / (m1 + m2)
            resultado4 = round(x, 3)
            if resultado4 >= 0:
                label_resultado.config(text=f"La aceleración es: {resultado4}(m/s²)")
                label_resultado10.config(text=f"La aceleración a favor \n de la masa2 es: {resultado4}(m/s²)")
            else:
                label_resultado.config(text=f"Este caso es imposible")
                label_resultado10.config(text=f"Este caso es imposible")
        except ValueError:
            label_resultado.config(text=f"Todos los valores deben \n ser unicamente numéricos")
    
    def calcular_tension():

        try:
            if masa1.get() != '':
                m1 = float(masa1.get())
                g = float(Gravedad.get())
                cos1 = float(angulo_de_inclinacion1.get())
                mu1 = float(friccion1.get())
                a = float(aceleracion.get())
                z = m1 * a
                y = m1 * g * math.sin(math.radians(cos1))
                w = mu1 * m1 * g * math.cos(math.radians(cos1))
                x = z + y + w
                resultado4 = round(x, 3)
                label_resultado.config(text=f"El resultado es: {resultado4}(N)")
                label_resultado11.config(text=f"La tension es de: {resultado4}(N)")

            else:
                m2 = float(masa2.get())
                g = float(Gravedad.get())
                cos2 = float(angulo_de_inclinacion2.get())
                mu2 = float(friccion2.get())
                a = float(aceleracion.get())
                z = m2 * a
                y = m2 * g * math.sin(math.radians(cos2))
                w = mu2 * m2 * g * math.cos(math.radians(cos2))
                x = z + y + w
                resultado4 = round(x, 3)
                label_resultado.config(text=f"El resultado es: {resultado4}(N)")
                label_resultado11.config(text=f"La tension es de: {resultado4}(N)")
        except ValueError:
            label_resultado.config(text=f"Todos los valores deben \n ser unicamente numéricos")

    # Borrar todos los datos laterales guardados
    def limpiar_barra_lateral():
        label_resultado1.config(text=f" ")
        label_resultado2.config(text=f" ")
        label_resultado3.config(text=f" ")
        label_resultado4.config(text=f" ")
        label_resultado5.config(text=f" ")
        label_resultado6.config(text=f" ")
        label_resultado7.config(text=f" ")
        label_resultado8.config(text=f" ")
        label_resultado9.config(text=f" ")
        label_resultado10.config(text=f" ")
        label_resultado11.config(text=f" ")

    # Crear el botón de calcular
    boton_calcular_P1 = ctk.CTkButton(ventana2, text="Calcular P1", command=calcular_Peso1 ,text_color="white")
    boton_calcular_P1.grid(column=0, row=5, padx=5, pady=5)
    boton_calcular_P2 = ctk.CTkButton(ventana2, text="Calcular P2", command=calcular_Peso2 ,text_color="white")
    boton_calcular_P2.grid(column=0, row=6, padx=5, pady=5)
    boton_calcular_Fr1 = ctk.CTkButton(ventana2, text="Calcular Fr1", command=calcular_Friccion1 ,text_color="white")
    boton_calcular_Fr1.grid(column=1, row=5, padx=5, pady=5)
    boton_calcular_Fr2 = ctk.CTkButton(ventana2, text="Calcular Fr2", command=calcular_Friccion2 ,text_color="white")
    boton_calcular_Fr2.grid(column=1, row=6, padx=5, pady=5)
    boton_calcular_Py1 = ctk.CTkButton(ventana2, text="Calcular Py1", command=calcular_Pesoy1 ,text_color="white")
    boton_calcular_Py1.grid(column=2, row=5, padx=5, pady=5)
    boton_calcular_Px1 = ctk.CTkButton(ventana2, text="Calcular Px1", command=calcular_Pesox1 ,text_color="white")
    boton_calcular_Px1.grid(column=3, row=5, padx=5, pady=5)
    boton_calcular_Py2 = ctk.CTkButton(ventana2, text="Calcular Py2", command=calcular_Pesoy2 ,text_color="white")
    boton_calcular_Py2.grid(column=2, row=6, padx=5, pady=5)
    boton_calcular_Px2 = ctk.CTkButton(ventana2, text="Calcular Px2", command=calcular_Pesox2 ,text_color="white")
    boton_calcular_Px2.grid(column=3, row=6, padx=5, pady=5)
    boton_calcular_a = ctk.CTkButton(ventana2, text="Calcular 'a' \n a favor de m1", command=calcular_aceleraciónfm1 ,text_color="white")
    boton_calcular_a.grid(column=4, row=5, padx=5, pady=5)
    boton_calcular_a = ctk.CTkButton(ventana2, text="Calcular 'a' \n a favor de m2", command=calcular_aceleraciónfm2 ,text_color="white")
    boton_calcular_a.grid(column=4, row=6, padx=5, pady=5)
    boton_calcular_T = ctk.CTkButton(ventana2, text="Calcular T", command=calcular_tension, text_color="white")
    boton_calcular_T.grid(column=5, row=6, padx=5, pady=5)
    boton_borrar = ctk.CTkButton(ventana2, text="Borrar datos laterales", command=limpiar_barra_lateral ,text_color="white")
    boton_borrar.grid(column=5, row=5, padx=5, pady=5)

    def cerrar_ventana():
        ventana2.destroy()

    btn_cerrar = ctk.CTkButton(ventana2, text="                                Cerrar ventana                                ", font=("Helvetica", 10, "bold"), command=cerrar_ventana, text_color="white")
    btn_cerrar.grid(column=0, row=9, columnspan=3, padx=5, pady=5)

    # despliege de secciones
    def seleccionar_opcion(event):
        opcion_seleccionada = combobox.get()

        if opcion_seleccionada == "1":
            pop()
        elif opcion_seleccionada == "2":
            pop1()

    opciones = ["1", "2"]
    combobox = ctk.CTkComboBox(ventana2, values=opciones)
    combobox.grid(row=8, column=1, padx=5, pady=5)
    combobox.bind("<<ComboboxSelected>>", seleccionar_opcion)
    etiqueta_seleccion = ctk.CTkLabel(ventana2, text="Cual es la masa que cae: \n 1. A favor de la masa 1 \n 2. A favor de la masa 2", text_color="white")
    etiqueta_seleccion.grid(row=8, column=0, padx=5, pady=5)

    etiqueta_angulo_de_inclinacion1 = ctk.CTkLabel(ventana2, text="Algulo de inclinación 1:",text_color="white")
    etiqueta_angulo_de_inclinacion1.grid(column=0, row=4, padx=5, pady=5)
    angulo_de_inclinacion1 = (ctk.CTkEntry(ventana2))
    angulo_de_inclinacion1.grid(column=1, row=4, padx=5, pady=5)

    etiqueta_angulo_de_inclinacion2 = ctk.CTkLabel(ventana2, text="Algulo de inclinación 2:",text_color="white")
    etiqueta_angulo_de_inclinacion2.grid(column=2, row=4, padx=5, pady=5)
    angulo_de_inclinacion2 = (ctk.CTkEntry(ventana2))
    angulo_de_inclinacion2.grid(column=3, row=4, padx=5, pady=5)


    def pop():
        
        text1 = 'Masa 1'
        text2 = 'Masa 2'
        xz = float(angulo_de_inclinacion1.get())
        xy = float(angulo_de_inclinacion2.get())
        lienzo_turtle = turtle.ScrolledCanvas(ventana2)
        lienzo_turtle.grid(column=3, row=7, columnspan=4, rowspan=7, padx=10, pady=5)
        # Crear un objeto Turtle en el lienzo
        t = turtle.RawTurtle(lienzo_turtle)

        if xz >= 90 and xy >= 31:

            t.speed(100)

            a1 = float(angulo_de_inclinacion1.get())
            a2 = float(angulo_de_inclinacion2.get())
            a3 = 360 - (a1 + a2)

            l1 = 200
            l2 = (math.sin(math.radians(a1)) * 200) / math.sin(math.radians(a2))
            l3 = (math.sin(math.radians(a3)) * 200) / math.sin(math.radians(a2))

            #Triangulo de dos massas
            t.pensize(4)
            t.pencolor('grey')

            t.penup()
            t.setpos(150, -110)
            t.pendown()

            t.left(-a1)
            t.backward(l1)
            t.right(a3)
            t.backward(l2)
            t.right(a2)
            t.backward(l3)

            # Polea
            t.left(-a1)
            t.backward(l1)
            t.left(a1)
            t.pencolor('black')
            t.left(90)
            t.forward(10)
            t.penup()
            t.forward(10)
            t.pendown()
            t.forward(1)
            t.backward(1)
            t.left(45)
            t.penup()
            t.forward(10)
            t.pendown()
            t.left(90)
            t.circle(10, 270)

            # Cuerda
            t.pencolor('violet')
            t.right(45)
            t.penup()
            t.forward(10)
            t.left(180)
            t.forward(10)
            t.pendown()
            t.left(-90)
            t.right(a1)
            t.backward((l1 / 2) + 20)

            # Massa2
            t.pencolor('black')
            t.left(90)
            t.forward(5)
            t.backward(50)
            t.left(90)
            t.forward(50)
            t.right(90)
            t.forward(50)
            t.right(90)
            t.forward(50)
            t.right(90)
            t.forward(25)
            t.penup()
            t.right(90)
            t.forward(25)
            t.pendown()
            t.forward(1)
            t.backward(1)
            t.penup()
            t.forward(25)

            # Linea de ejes M2
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)

            # Letra Y del eje
            t.penup()
            t.forward(5)
            t.pendown()
            t.left(45)
            t.forward(10)
            t.left(90)
            t.forward(10)
            t.left(180)
            t.forward(20)
            t.right(180)
            t.forward(10)
            t.left(90)
            t.forward(10)
            t.right(45)
            t.penup()
            t.forward(5)
            t.pendown()
                    
            # Linea de ejes M2
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.forward(25)
            t.left(90)
            t.forward(25)
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.left(180)
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.forward(50)
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
                    
            # Letra X del eje
            t.penup()
            t.forward(5)
            t.pendown()
            t.left(45)
            t.forward(10)
            t.left(90)
            t.forward(10)
            t.left(180)
            t.forward(20)
            t.right(180)
            t.forward(10)
            t.right(90)
            t.forward(10)
            t.left(180)
            t.forward(20)
            t.right(45)
            t.penup()
            t.forward(5)
            t.pendown()
                    
            # Linea de ejes M2
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(30)

            # Flecha Fuerza (N)
            t.pencolor('blue')
            t.left(180)
            t.forward(25)
            t.pendown()
            t.forward(40)
            t.left(90)
            t.forward(10)
            t.right(135)
            t.forward(15)
            t.right(180)
            t.forward(15)
            t.left(135)
            t.forward(20)
            t.left(135)
            t.forward(15)
            t.left(180)
            t.forward(15)
            t.right(135)
            t.forward(10)
            t.left(90)
            t.forward(40)

            #Traslado
            t.penup()
            t.forward(25)
            t.right(90)
            t.forward(25)

            # Flecha Fuerza (Fr)
            t.pencolor('red')
            t.pendown()
            t.forward(40)
            t.left(90)
            t.forward(10)
            t.right(135)
            t.forward(15)
            t.right(180)
            t.forward(15)
            t.left(135)
            t.forward(20)
            t.left(135)
            t.forward(15)
            t.left(180)
            t.forward(15)
            t.right(135)
            t.forward(10)
            t.left(90)
            t.forward(40)

            #Traslado
            t.penup()
            t.forward(25)
            t.right(-a1)
            t.right(90)
            t.forward(25)

            # Flecha Fuerza (P)
            t.pencolor('gold')
            t.pendown()
            t.forward(40)
            t.left(90)
            t.forward(10)
            t.right(135)
            t.forward(15)
            t.right(180)
            t.forward(15)
            t.left(135)
            t.forward(20)
            t.left(135)
            t.forward(15)
            t.left(180)
            t.forward(15)
            t.right(135)
            t.forward(10)
            t.left(90)
            t.forward(40)

            #Traslado
            t.penup()
            t.forward(25)
            t.left(90)
            t.left(-a1)

            # Cuerda
            t.penup()
            t.forward(25)
            t.left(90)
            t.forward(20)
            t.right(90)
            t.forward((l1 / 2) + 20)
            t.right(-a1)
            t.left(180)
            t.forward(10)
            t.left(180)
            t.forward(10)
            t.left(45)
            t.forward(10)
            t.right(90)
            t.forward(10)
            t.left(45)
            t.pendown()
            t.pencolor('violet')
            t.right(-a2)
            t.left(180)
            t.backward((l2 / 2) + 20)

            # Massa1
            t.pencolor('black')
            t.left(90)
            t.forward(27)
            t.backward(50)
            t.left(90)
            t.forward(50)
            t.right(90)
            t.forward(50)
            t.right(90)
            t.forward(50)
            t.right(90)
            t.forward(27)
            t.penup()
            t.right(90)
            t.forward(25)
            t.pendown()
            t.forward(1)
            t.backward(1)
            t.penup()
            t.forward(25)

            # Linea de ejes M1
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
                    
            # Letra Y del eje
            t.penup()
            t.forward(5)
            t.pendown()
            t.right(45)
            t.forward(10)
            t.left(90)
            t.forward(10)
            t.left(180)
            t.forward(20)
            t.right(180)
            t.forward(10)
            t.right(90)
            t.forward(10)
            t.left(180)
            t.forward(20)
            t.left(45)
            t.penup()
            t.forward(5)
            t.pendown()
                    
            # Linea de ejes M1
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.forward(25)
            t.left(90)
            t.forward(25)
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
                    
            # Letra Y del eje
            t.penup()
            t.forward(5)
            t.pendown()
            t.right(45)
            t.forward(20)
            t.left(180)
            t.forward(10)
            t.right(90)
            t.forward(10)
            t.right(180)
            t.forward(10)
            t.right(90)
            t.forward(10)
            t.left(45)
            t.penup()
            t.forward(5)
            t.pendown()
                    
            # Linea de ejes M1
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.forward(50)
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.left(180)
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(30)

            # Flecha Fuerza (N2)
            t.pencolor('blue')
            t.forward(25)
            t.pendown()
            t.forward(40)
            t.left(90)
            t.forward(10)
            t.right(135)
            t.forward(15)
            t.right(180)
            t.forward(15)
            t.left(135)
            t.forward(20)
            t.left(135)
            t.forward(15)
            t.left(180)
            t.forward(15)
            t.right(135)
            t.forward(10)
            t.left(90)
            t.forward(40)

            #Traslado2
            t.penup()
            t.forward(25)
            t.right(90)
            t.forward(25)

            # Flecha Fuerza (Fr2)
            t.pencolor('red')
            t.pendown()
            t.forward(40)
            t.left(90)
            t.forward(10)
            t.right(135)
            t.forward(15)
            t.right(180)
            t.forward(15)
            t.left(135)
            t.forward(20)
            t.left(135)
            t.forward(15)
            t.left(180)
            t.forward(15)
            t.right(135)
            t.forward(10)
            t.left(90)
            t.forward(40)

            #Traslado2
            t.penup()
            t.forward(25)
            t.right(a2)
            t.right(90)
            t.forward(25)

            # Flecha Fuerza (P2)
            t.pencolor('gold')
            t.pendown()
            t.forward(40)
            t.left(90)
            t.forward(10)
            t.right(135)
            t.forward(15)
            t.right(180)
            t.forward(15)
            t.left(135)
            t.forward(20)
            t.left(135)
            t.forward(15)
            t.left(180)
            t.forward(15)
            t.right(135)
            t.forward(10)
            t.left(90)
            t.forward(40)

            #Traslado2
            t.penup()
            t.forward(25)
            t.left(90)
            t.left(a2)

            # Polea
            t.right(180)
            t.forward(25)
            t.forward((l2 / 2) + 20)
            t.right(a2)
            t.right(45)
            t.forward(10)
            t.right(135)
            t.forward(10)
            t.right(90)
            t.pendown()
            t.pencolor('black')
            t.circle(-10)


            # Cuadro de leyendass
            t.pensize(3)
            t.pencolor('black')

            t.penup()
            t.setpos(-150, 25)
            t.pendown()

            # Quadrado
            t.settiltangle(0)
            t.forward(50)

            # Primera linea divisoria
            t.left(90)
            t.forward(30)

            # Letra F
            t.pencolor('lightgreen')
            t.right(90)
            t.forward(30)
            t.right(90)
            t.forward(15)
            t.right(180)
            t.forward(15)
            t.left(90)
            t.forward(10)
            t.left(90)
            t.forward(10)
            t.left(180)
            t.forward(10)
            t.left(90)
            t.forward(20)

            # Primera linea divisoria
            t.pencolor('black')
            t.left(270)
            t.forward(55)

            # Letra F de Fr
            t.pencolor('red')
            t.right(90)
            t.forward(30)
            t.right(90)
            t.forward(15)
            t.right(180)
            t.forward(15)
            t.left(90)
            t.forward(10)
            t.left(90)
            t.forward(10)
            t.left(180)
            t.forward(10)
            t.left(90)
            t.forward(20)

            # Primera linea divisoria
            t.pencolor('black')
            t.left(270)
            t.forward(15)
            t.left(180)
            t.forward(25)

            # Letra R de Fr
            t.pencolor('red')
            t.left(90)
            t.forward(16)
            t.left(180)
            t.forward(4)
            t.left(90)
            t.forward(7)
            t.right(90)
            t.forward(3)
            t.left(180)
            t.forward(3)
            t.left(90)
            t.forward(7)
            t.left(90)
            t.forward(12)

            # Primera linea divisoria
            t.pencolor('black')
            t.left(90)
            t.forward(75)

            # Quadrado
            t.left(90)
            t.forward(50)
            t.left(90)
            t.forward(100)
            t.left(90)
            t.forward(100)
            t.left(90)
            t.forward(15)

            # Letra N
            t.pencolor('blue')
            t.left(90)
            t.forward(30)
            t.left(215)
            t.forward(37)
            t.right(215)
            t.forward(30)
            t.right(180)
            t.forward(30)
            t.left(215)
            t.forward(37)
            t.right(215)
            t.forward(30)

            # Quadrado
            t.pencolor('black')
            t.left(90)
            t.forward(35)

            # Segunda linea divisoria
            t.left(90)
            t.forward(100)
            t.left(180)
            t.forward(100)

            # Quadrado
            t.left(90)
            t.forward(20)

            # Letra P
            t.pencolor('Gold')
            t.left(90)
            t.forward(30)
            t.right(90)
            t.forward(15)
            t.right(90)
            t.forward(15)
            t.right(90)
            t.forward(15)
            t.left(90)
            t.forward(15)

            # Quadrado
            t.pencolor('black')
            t.left(90)
            t.forward(30)
            t.left(90)
            t.forward(150)
            t.left(90)
            t.forward(100)
            t.left(90)
            t.forward(50)
            t.left(90)
            t.forward(50)

            # Letra T
            t.left(90)
            t.forward(2)
            t.pencolor('violet')
            t.forward(35)
            t.left(90)
            t.forward(20)
            t.left(180)
            t.forward(40)

            # cual es cada massa
            t.pencolor('black')
            x = 200
            y = 130
            t.penup()
            t.goto(x, y)
            t.write(text1, align="center", font=('Courier New', 10))

            x = 0
            y = 130
            t.goto(x, y)
            t.write(text2, align="center", font=('Courier New', 10))

            # Esconder el lapiz
            t.penup()
            t.setpos(-320, -280)
            t.pendown()
            t.forward(1)
            t.hideturtle()
                
        elif xz >= 31 and xy >= 90:

            t.speed(100)

            a1 = float(angulo_de_inclinacion1.get())
            a2 = float(angulo_de_inclinacion2.get())
            a3 = 360 - (a1 + a2)

            l1 = 300
            l2 = (math.sin(math.radians(a1)) * 300) / math.sin(math.radians(a2))
            l3 = (math.sin(math.radians(a3)) * 300) / math.sin(math.radians(a2))

            #Triangulo de dos massas
            t.pensize(4)
            t.pencolor('grey')

            t.penup()
            t.setpos(180, -120)
            t.pendown()

            t.left(-a1)
            t.backward(l1)
            t.right(a3)
            t.backward(l2)
            t.right(a2)
            t.backward(l3)

            # Polea
            t.left(-a1)
            t.backward(l1)
            t.left(a1)
            t.pencolor('black')
            t.left(90)
            t.forward(10)
            t.penup()
            t.forward(10)
            t.pendown()
            t.forward(1)
            t.backward(1)
            t.left(45)
            t.penup()
            t.forward(10)
            t.pendown()
            t.left(90)
            t.circle(10, 270)

            # Cuerda
            t.pencolor('violet')
            t.right(45)
            t.penup()
            t.forward(10)
            t.left(180)
            t.forward(10)
            t.pendown()
            t.left(-90)
            t.right(a1)
            t.backward((l1 / 2) + 20)

            # Massa2
            t.pencolor('black')
            t.left(90)
            t.forward(25)
            t.backward(50)
            t.left(90)
            t.forward(50)
            t.right(90)
            t.forward(50)
            t.right(90)
            t.forward(50)
            t.right(90)
            t.forward(25)
            t.penup()
            t.right(90)
            t.forward(25)
            t.pendown()
            t.forward(1)
            t.backward(1)
            t.penup()
            t.forward(25)

            # Linea de ejes M2
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)

            # Letra X del eje
            t.penup()
            t.forward(5)
            t.pendown()
            t.left(45)
            t.forward(10)
            t.left(90)
            t.forward(10)
            t.left(180)
            t.forward(20)
            t.right(180)
            t.forward(10)
            t.right(90)
            t.forward(10)
            t.left(180)
            t.forward(20)
            t.right(45)
            t.penup()
            t.forward(5)
            t.pendown()

            # Linea de ejes M2
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.forward(25)
            t.left(90)
            t.forward(25)
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.left(180)
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.forward(50)
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)

            # Letra Y del eje
            t.penup()
            t.forward(5)
            t.pendown()
            t.right(45)
            t.forward(20)
            t.left(180)
            t.forward(10)
            t.right(90)
            t.forward(10)
            t.right(180)
            t.forward(10)
            t.right(90)
            t.forward(10)
            t.left(45)
            t.penup()
            t.forward(5)
            t.pendown()                    
                    
            # Linea de ejes M2
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(30)

            # Flecha Fuerza (N)
            t.pencolor('blue')
            t.left(180)
            t.forward(25)
            t.pendown()
            t.forward(40)
            t.left(90)
            t.forward(10)
            t.right(135)
            t.forward(15)
            t.right(180)
            t.forward(15)
            t.left(135)
            t.forward(20)
            t.left(135)
            t.forward(15)
            t.left(180)
            t.forward(15)
            t.right(135)
            t.forward(10)
            t.left(90)
            t.forward(40)

            #Traslado
            t.penup()
            t.forward(25)
            t.right(90)
            t.forward(25)

            # Flecha Fuerza (Fr)
            t.pencolor('red')
            t.pendown()
            t.forward(40)
            t.left(90)
            t.forward(10)
            t.right(135)
            t.forward(15)
            t.right(180)
            t.forward(15)
            t.left(135)
            t.forward(20)
            t.left(135)
            t.forward(15)
            t.left(180)
            t.forward(15)
            t.right(135)
            t.forward(10)
            t.left(90)
            t.forward(40)

            #Traslado
            t.penup()
            t.forward(25)
            t.right(-a1)
            t.right(90)
            t.forward(25)

            # Flecha Fuerza (P)
            t.pencolor('gold')
            t.pendown()
            t.forward(40)
            t.left(90)
            t.forward(10)
            t.right(135)
            t.forward(15)
            t.right(180)
            t.forward(15)
            t.left(135)
            t.forward(20)
            t.left(135)
            t.forward(15)
            t.left(180)
            t.forward(15)
            t.right(135)
            t.forward(10)
            t.left(90)
            t.forward(40)

            #Traslado
            t.penup()
            t.forward(25)
            t.left(90)
            t.left(-a1)

            # Cuerda
            t.penup()
            t.forward(25)
            t.forward((l1 / 2) + 20)
            t.right(-a1)
            t.left(180)
            t.forward(10)
            t.left(180)
            t.forward(10)
            t.left(45)
            t.forward(10)
            t.right(90)
            t.forward(10)
            t.left(45)
            t.pendown()
            t.pencolor('violet')
            t.right(-a2)
            t.left(180)
            t.backward((l2 / 2) + 20)

            # Massa1
            t.pencolor('black')
            t.left(90)
            t.forward(45)
            t.backward(50)
            t.left(90)
            t.forward(50)
            t.right(90)
            t.forward(50)
            t.right(90)
            t.forward(50)
            t.right(90)
            t.forward(25)
            t.penup()
            t.right(90)
            t.forward(25)
            t.pendown()
            t.forward(1)
            t.backward(1)
            t.penup()
            t.forward(25)

            # Linea de ejes M1
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
                    
            # Letra Y del eje
            t.penup()
            t.forward(5)
            t.pendown()
            t.left(45)
            t.forward(10)
            t.left(90)
            t.forward(10)
            t.left(180)
            t.forward(20)
            t.right(180)
            t.forward(10)
            t.left(90)
            t.forward(10)
            t.right(45)
            t.penup()
            t.forward(5)
            t.pendown()
                    
            # Linea de ejes M1
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.forward(25)
            t.left(90)
            t.forward(25)
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
                    
            # Letra X del eje
            t.penup()
            t.forward(5)
            t.pendown()
            t.right(45)
            t.forward(10)
            t.left(90)
            t.forward(10)
            t.left(180)
            t.forward(20)
            t.right(180)
            t.forward(10)
            t.right(90)
            t.forward(10)
            t.left(180)
            t.forward(20)
            t.left(45)
            t.penup()
            t.forward(5)
            t.pendown()
                    
            # Linea de ejes M1
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.forward(50)
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.left(180)
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(30)

            # Flecha Fuerza (N2)
            t.pencolor('blue')
            t.forward(25)
            t.pendown()
            t.forward(40)
            t.left(90)
            t.forward(10)
            t.right(135)
            t.forward(15)
            t.right(180)
            t.forward(15)
            t.left(135)
            t.forward(20)
            t.left(135)
            t.forward(15)
            t.left(180)
            t.forward(15)
            t.right(135)
            t.forward(10)
            t.left(90)
            t.forward(40)

            #Traslado2
            t.penup()
            t.forward(25)
            t.right(90)
            t.forward(25)

            # Flecha Fuerza (Fr2)
            t.pencolor('red')
            t.pendown()
            t.forward(40)
            t.left(90)
            t.forward(10)
            t.right(135)
            t.forward(15)
            t.right(180)
            t.forward(15)
            t.left(135)
            t.forward(20)
            t.left(135)
            t.forward(15)
            t.left(180)
            t.forward(15)
            t.right(135)
            t.forward(10)
            t.left(90)
            t.forward(40)

            #Traslado2
            t.penup()
            t.forward(25)
            t.right(a2)
            t.right(90)
            t.forward(25)

            # Flecha Fuerza (P2)
            t.pencolor('gold')
            t.pendown()
            t.forward(40)
            t.left(90)
            t.forward(10)
            t.right(135)
            t.forward(15)
            t.right(180)
            t.forward(15)
            t.left(135)
            t.forward(20)
            t.left(135)
            t.forward(15)
            t.left(180)
            t.forward(15)
            t.right(135)
            t.forward(10)
            t.left(90)
            t.forward(40)

            #Traslado2
            t.penup()
            t.forward(25)
            t.left(90)
            t.left(a2)

            # Polea
            t.right(180)
            t.forward(25)
            t.right(90)
            t.forward(20)
            t.left(90)
            t.forward((l2 / 2) + 20)
            t.right(a2)
            t.right(45)
            t.forward(10)
            t.right(135)
            t.forward(10)
            t.right(90)
            t.pendown()
            t.pencolor('black')
            t.circle(-10)


            # Cuadro de leyendass
            t.pensize(3)
            t.pencolor('black')

            t.penup()
            t.setpos(-150, 25)
            t.pendown()

            # Quadrado
            t.settiltangle(0)
            t.forward(50)

            # Primera linea divisoria
            t.left(90)
            t.forward(30)

            # Letra F
            t.pencolor('lightgreen')
            t.right(90)
            t.forward(30)
            t.right(90)
            t.forward(15)
            t.right(180)
            t.forward(15)
            t.left(90)
            t.forward(10)
            t.left(90)
            t.forward(10)
            t.left(180)
            t.forward(10)
            t.left(90)
            t.forward(20)

            # Primera linea divisoria
            t.pencolor('black')
            t.left(270)
            t.forward(55)

            # Letra F de Fr
            t.pencolor('red')
            t.right(90)
            t.forward(30)
            t.right(90)
            t.forward(15)
            t.right(180)
            t.forward(15)
            t.left(90)
            t.forward(10)
            t.left(90)
            t.forward(10)
            t.left(180)
            t.forward(10)
            t.left(90)
            t.forward(20)

            # Primera linea divisoria
            t.pencolor('black')
            t.left(270)
            t.forward(15)
            t.left(180)
            t.forward(25)

            # Letra R de Fr
            t.pencolor('red')
            t.left(90)
            t.forward(16)
            t.left(180)
            t.forward(4)
            t.left(90)
            t.forward(7)
            t.right(90)
            t.forward(3)
            t.left(180)
            t.forward(3)
            t.left(90)
            t.forward(7)
            t.left(90)
            t.forward(12)

            # Primera linea divisoria
            t.pencolor('black')
            t.left(90)
            t.forward(75)

            # Quadrado
            t.left(90)
            t.forward(50)
            t.left(90)
            t.forward(100)
            t.left(90)
            t.forward(100)
            t.left(90)
            t.forward(15)

            # Letra N
            t.pencolor('blue')
            t.left(90)
            t.forward(30)
            t.left(215)
            t.forward(37)
            t.right(215)
            t.forward(30)
            t.right(180)
            t.forward(30)
            t.left(215)
            t.forward(37)
            t.right(215)
            t.forward(30)

            # Quadrado
            t.pencolor('black')
            t.left(90)
            t.forward(35)

            # Segunda linea divisoria
            t.left(90)
            t.forward(100)
            t.left(180)
            t.forward(100)

            # Quadrado
            t.left(90)
            t.forward(20)

            # Letra P
            t.pencolor('Gold')
            t.left(90)
            t.forward(30)
            t.right(90)
            t.forward(15)
            t.right(90)
            t.forward(15)
            t.right(90)
            t.forward(15)
            t.left(90)
            t.forward(15)

            # Quadrado
            t.pencolor('black')
            t.left(90)
            t.forward(30)
            t.left(90)
            t.forward(150)
            t.left(90)
            t.forward(100)
            t.left(90)
            t.forward(50)
            t.left(90)
            t.forward(50)

            # Letra T
            t.left(90)
            t.forward(2)
            t.pencolor('violet')
            t.forward(35)
            t.left(90)
            t.forward(20)
            t.left(180)
            t.forward(40)

            # cual es cada massa
            t.pencolor('black')
            x = 160
            y = 150
            t.penup()
            t.goto(x, y)
            t.write(text1, align="center", font=('Courier New', 10))

            x = -60
            y = 150
            t.goto(x, y)
            t.write(text2, align="center", font=('Courier New', 10))

            # Esconder el lapiz
            t.penup()
            t.setpos(-320, -280)
            t.pendown()
            t.forward(1)
            t.hideturtle()
        
        elif xz >= 51 and xy >= 51:

            t.speed(100)

            a1 = float(angulo_de_inclinacion1.get())
            a2 = float(angulo_de_inclinacion2.get())
            a3 = 360 - (a1 + a2)

            l1 = 250
            l2 = (math.sin(math.radians(a1)) * 250) / math.sin(math.radians(a2))
            l3 = (math.sin(math.radians(a3)) * 250) / math.sin(math.radians(a2))

            #Triangulo de dos massas
            t.pensize(4)
            t.pencolor('grey')

            t.penup()
            t.setpos(170, -160)
            t.pendown()

            t.left(-a1)
            t.backward(l1)
            t.right(a3)
            t.backward(l2)
            t.right(a2)
            t.backward(l3)

            # Polea
            t.left(-a1)
            t.backward(l1)
            t.left(a1)
            t.pencolor('black')
            t.left(90)
            t.forward(10)
            t.penup()
            t.forward(10)
            t.pendown()
            t.forward(1)
            t.backward(1)
            t.left(45)
            t.penup()
            t.forward(10)
            t.pendown()
            t.left(90)
            t.circle(10, 270)

            # Cuerda
            t.pencolor('violet')
            t.right(45)
            t.penup()
            t.forward(10)
            t.left(180)
            t.forward(10)
            t.pendown()
            t.left(-90)
            t.right(a1)
            t.backward((l1 / 2) + 20)

            # Massa2
            t.pencolor('black')
            t.left(90)
            t.forward(25)
            t.backward(50)
            t.left(90)
            t.forward(50)
            t.right(90)
            t.forward(50)
            t.right(90)
            t.forward(50)
            t.right(90)
            t.forward(25)
            t.penup()
            t.right(90)
            t.forward(25)
            t.pendown()
            t.forward(1)
            t.backward(1)
            t.penup()
            t.forward(25)

            # Linea de ejes M2
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)

            # Letra X del eje
            t.penup()
            t.forward(5)
            t.pendown()
            t.left(45)
            t.forward(10)
            t.left(90)
            t.forward(10)
            t.left(180)
            t.forward(20)
            t.right(180)
            t.forward(10)
            t.right(90)
            t.forward(10)
            t.left(180)
            t.forward(20)
            t.right(45)
            t.penup()
            t.forward(5)
            t.pendown()

            # Linea de ejes M2
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.forward(25)
            t.left(90)
            t.forward(25)
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.left(180)
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.forward(50)
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
                    
            # Letra Y del eje
            t.penup()
            t.forward(5)
            t.pendown()
            t.right(45)
            t.forward(20)
            t.left(180)
            t.forward(10)
            t.right(90)
            t.forward(10)
            t.right(180)
            t.forward(10)
            t.right(90)
            t.forward(10)
            t.left(45)
            t.penup()
            t.forward(5)
            t.pendown()                    
                    
            # Linea de ejes M2
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(30)

            # Flecha Fuerza (N)
            t.pencolor('blue')
            t.left(180)
            t.forward(25)
            t.pendown()
            t.forward(40)
            t.left(90)
            t.forward(10)
            t.right(135)
            t.forward(15)
            t.right(180)
            t.forward(15)
            t.left(135)
            t.forward(20)
            t.left(135)
            t.forward(15)
            t.left(180)
            t.forward(15)
            t.right(135)
            t.forward(10)
            t.left(90)
            t.forward(40)

            #Traslado
            t.penup()
            t.forward(25)
            t.right(90)
            t.forward(25)

            # Flecha Fuerza (Fr)
            t.pencolor('red')
            t.pendown()
            t.forward(40)
            t.left(90)
            t.forward(10)
            t.right(135)
            t.forward(15)
            t.right(180)
            t.forward(15)
            t.left(135)
            t.forward(20)
            t.left(135)
            t.forward(15)
            t.left(180)
            t.forward(15)
            t.right(135)
            t.forward(10)
            t.left(90)
            t.forward(40)

            #Traslado
            t.penup()
            t.forward(25)
            t.right(-a1)
            t.right(90)
            t.forward(25)

            # Flecha Fuerza (P)
            t.pencolor('gold')
            t.pendown()
            t.forward(40)
            t.left(90)
            t.forward(10)
            t.right(135)
            t.forward(15)
            t.right(180)
            t.forward(15)
            t.left(135)
            t.forward(20)
            t.left(135)
            t.forward(15)
            t.left(180)
            t.forward(15)
            t.right(135)
            t.forward(10)
            t.left(90)
            t.forward(40)

            #Traslado
            t.penup()
            t.forward(25)
            t.left(90)
            t.left(-a1)

            # Cuerda
            t.penup()
            t.forward(25)
            t.forward((l1 / 2) + 20)
            t.right(-a1)
            t.left(180)
            t.forward(10)
            t.left(180)
            t.forward(10)
            t.left(45)
            t.forward(10)
            t.right(90)
            t.forward(10)
            t.left(45)
            t.pendown()
            t.pencolor('violet')
            t.right(-a2)
            t.left(180)
            t.backward((l2 / 2) + 20)

            # Massa1
            t.pencolor('black')
            t.left(90)
            t.forward(27)
            t.backward(50)
            t.left(90)
            t.forward(50)
            t.right(90)
            t.forward(50)
            t.right(90)
            t.forward(50)
            t.right(90)
            t.forward(27)
            t.penup()
            t.right(90)
            t.forward(25)
            t.pendown()
            t.forward(1)
            t.backward(1)
            t.penup()
            t.forward(25)

            # Linea de ejes M1
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
                    
            # Letra X del eje
            t.penup()
            t.forward(5)
            t.pendown()
            t.right(45)
            t.forward(10)
            t.left(90)
            t.forward(10)
            t.left(180)
            t.forward(20)
            t.right(180)
            t.forward(10)
            t.right(90)
            t.forward(10)
            t.left(180)
            t.forward(20)
            t.left(45)
            t.penup()
            t.forward(5)
            t.pendown()

            # Linea de ejes M1
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.forward(25)
            t.left(90)
            t.forward(25)
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
                    
            # Letra Y del eje
            t.penup()
            t.forward(5)
            t.pendown()
            t.right(45)
            t.forward(20)
            t.left(180)
            t.forward(10)
            t.right(90)
            t.forward(10)
            t.right(180)
            t.forward(10)
            t.right(90)
            t.forward(10)
            t.left(45)
            t.penup()
            t.forward(5)
            t.pendown()                    
                    
            # Linea de ejes M1
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.forward(50)
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.left(180)
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(30)

            # Flecha Fuerza (N2)
            t.pencolor('blue')
            t.forward(25)
            t.pendown()
            t.forward(40)
            t.left(90)
            t.forward(10)
            t.right(135)
            t.forward(15)
            t.right(180)
            t.forward(15)
            t.left(135)
            t.forward(20)
            t.left(135)
            t.forward(15)
            t.left(180)
            t.forward(15)
            t.right(135)
            t.forward(10)
            t.left(90)
            t.forward(40)

            #Traslado2
            t.penup()
            t.forward(25)
            t.right(90)
            t.forward(25)

            # Flecha Fuerza (Fr2)
            t.pencolor('red')
            t.pendown()
            t.forward(40)
            t.left(90)
            t.forward(10)
            t.right(135)
            t.forward(15)
            t.right(180)
            t.forward(15)
            t.left(135)
            t.forward(20)
            t.left(135)
            t.forward(15)
            t.left(180)
            t.forward(15)
            t.right(135)
            t.forward(10)
            t.left(90)
            t.forward(40)

            #Traslado2
            t.penup()
            t.forward(25)
            t.right(a2)
            t.right(90)
            t.forward(25)

            # Flecha Fuerza (P2)
            t.pencolor('gold')
            t.pendown()
            t.forward(40)
            t.left(90)
            t.forward(10)
            t.right(135)
            t.forward(15)
            t.right(180)
            t.forward(15)
            t.left(135)
            t.forward(20)
            t.left(135)
            t.forward(15)
            t.left(180)
            t.forward(15)
            t.right(135)
            t.forward(10)
            t.left(90)
            t.forward(40)

            #Traslado2
            t.penup()
            t.forward(25)
            t.left(90)
            t.left(a2)

            # Polea
            t.right(180)
            t.forward(25)
            t.forward((l2 / 2) + 20)
            t.right(a2)
            t.right(45)
            t.forward(10)
            t.right(135)
            t.forward(10)
            t.right(90)
            t.pendown()
            t.pencolor('black')
            t.circle(-10)


            # Cuadro de leyendass
            t.pensize(3)
            t.pencolor('black')

            t.penup()
            t.setpos(-150, 25)
            t.pendown()

            # Quadrado
            t.settiltangle(0)
            t.forward(50)

            # Primera linea divisoria
            t.left(90)
            t.forward(30)

            # Letra F
            t.pencolor('lightgreen')
            t.right(90)
            t.forward(30)
            t.right(90)
            t.forward(15)
            t.right(180)
            t.forward(15)
            t.left(90)
            t.forward(10)
            t.left(90)
            t.forward(10)
            t.left(180)
            t.forward(10)
            t.left(90)
            t.forward(20)

            # Primera linea divisoria
            t.pencolor('black')
            t.left(270)
            t.forward(55)

            # Letra F de Fr
            t.pencolor('red')
            t.right(90)
            t.forward(30)
            t.right(90)
            t.forward(15)
            t.right(180)
            t.forward(15)
            t.left(90)
            t.forward(10)
            t.left(90)
            t.forward(10)
            t.left(180)
            t.forward(10)
            t.left(90)
            t.forward(20)

            # Primera linea divisoria
            t.pencolor('black')
            t.left(270)
            t.forward(15)
            t.left(180)
            t.forward(25)

            # Letra R de Fr
            t.pencolor('red')
            t.left(90)
            t.forward(16)
            t.left(180)
            t.forward(4)
            t.left(90)
            t.forward(7)
            t.right(90)
            t.forward(3)
            t.left(180)
            t.forward(3)
            t.left(90)
            t.forward(7)
            t.left(90)
            t.forward(12)

            # Primera linea divisoria
            t.pencolor('black')
            t.left(90)
            t.forward(75)

            # Quadrado
            t.left(90)
            t.forward(50)
            t.left(90)
            t.forward(100)
            t.left(90)
            t.forward(100)
            t.left(90)
            t.forward(15)

            # Letra N
            t.pencolor('blue')
            t.left(90)
            t.forward(30)
            t.left(215)
            t.forward(37)
            t.right(215)
            t.forward(30)
            t.right(180)
            t.forward(30)
            t.left(215)
            t.forward(37)
            t.right(215)
            t.forward(30)

            # Quadrado
            t.pencolor('black')
            t.left(90)
            t.forward(35)

            # Segunda linea divisoria
            t.left(90)
            t.forward(100)
            t.left(180)
            t.forward(100)

            # Quadrado
            t.left(90)
            t.forward(20)

            # Letra P
            t.pencolor('Gold')
            t.left(90)
            t.forward(30)
            t.right(90)
            t.forward(15)
            t.right(90)
            t.forward(15)
            t.right(90)
            t.forward(15)
            t.left(90)
            t.forward(15)

            # Quadrado
            t.pencolor('black')
            t.left(90)
            t.forward(30)
            t.left(90)
            t.forward(150)
            t.left(90)
            t.forward(100)
            t.left(90)
            t.forward(50)
            t.left(90)
            t.forward(50)

            # Letra T
            t.left(90)
            t.forward(2)
            t.pencolor('violet')
            t.forward(35)
            t.left(90)
            t.forward(20)
            t.left(180)
            t.forward(40)

            # cual es cada massa
            t.pencolor('black')
            x = 130
            y = 80
            t.penup()
            t.goto(x, y)
            t.write(text1, align="center", font=('Courier New', 10))

            x = -80
            y = 80
            t.goto(x, y)
            t.write(text2, align="center", font=('Courier New', 10))

            # Esconder el lapiz
            t.penup()
            t.setpos(-320, -280)
            t.pendown()
            t.forward(1)
            t.hideturtle()

        elif xz >= 90:

            t.speed(100)

            a1 = float(angulo_de_inclinacion1.get())
            a2 = float(angulo_de_inclinacion2.get())
            a3 = 360 - (a1 + a2)

            l1 = 100
            l2 = (math.sin(math.radians(a1)) * 100) / math.sin(math.radians(a2))
            l3 = (math.sin(math.radians(a3)) * 100) / math.sin(math.radians(a2))

            #Triangulo de dos massas
            t.pensize(4)
            t.pencolor('grey')

            t.penup()
            t.setpos(140, -80)
            t.pendown()

            t.left(-a1)
            t.backward(l1)
            t.right(a3)
            t.backward(l2)
            t.right(a2)
            t.backward(l3)

            # Polea
            t.left(-a1)
            t.backward(l1)
            t.left(a1)
            t.pencolor('black')
            t.left(90)
            t.forward(10)
            t.penup()
            t.forward(10)
            t.pendown()
            t.forward(1)
            t.backward(1)
            t.left(45)
            t.penup()
            t.forward(10)
            t.pendown()
            t.left(90)
            t.circle(10, 270)

            # Cuerda
            t.pencolor('violet')
            t.right(45)
            t.penup()
            t.forward(10)
            t.left(180)
            t.forward(10)
            t.pendown()
            t.left(-90)
            t.right(a1)
            t.backward((l1 / 2) + 20)

            # Massa2
            t.pencolor('black')
            t.left(90)
            t.forward(5)
            t.backward(50)
            t.left(90)
            t.forward(50)
            t.right(90)
            t.forward(50)
            t.right(90)
            t.forward(50)
            t.right(90)
            t.forward(25)
            t.penup()
            t.right(90)
            t.forward(25)
            t.pendown()
            t.forward(1)
            t.backward(1)
            t.penup()
            t.forward(25)

            # Linea de ejes M2
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)

            # Letra Y del eje
            t.penup()
            t.forward(5)
            t.pendown()
            t.left(45)
            t.forward(10)
            t.left(90)
            t.forward(10)
            t.left(180)
            t.forward(20)
            t.right(180)
            t.forward(10)
            t.left(90)
            t.forward(10)
            t.right(45)
            t.penup()
            t.forward(5)
            t.pendown()
                    
            # Linea de ejes M2
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.forward(25)
            t.left(90)
            t.forward(25)
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.left(180)
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.forward(50)
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
                    
            # Letra X del eje
            t.penup()
            t.forward(5)
            t.pendown()
            t.left(45)
            t.forward(10)
            t.left(90)
            t.forward(10)
            t.left(180)
            t.forward(20)
            t.right(180)
            t.forward(10)
            t.right(90)
            t.forward(10)
            t.left(180)
            t.forward(20)
            t.right(45)
            t.penup()
            t.forward(5)
            t.pendown()
                    
            # Linea de ejes M2
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(30)

            # Flecha Fuerza (N)
            t.pencolor('blue')
            t.left(180)
            t.forward(25)
            t.pendown()
            t.forward(40)
            t.left(90)
            t.forward(10)
            t.right(135)
            t.forward(15)
            t.right(180)
            t.forward(15)
            t.left(135)
            t.forward(20)
            t.left(135)
            t.forward(15)
            t.left(180)
            t.forward(15)
            t.right(135)
            t.forward(10)
            t.left(90)
            t.forward(40)

            #Traslado
            t.penup()
            t.forward(25)
            t.right(90)
            t.forward(25)

            # Flecha Fuerza (Fr)
            t.pencolor('red')
            t.pendown()
            t.forward(40)
            t.left(90)
            t.forward(10)
            t.right(135)
            t.forward(15)
            t.right(180)
            t.forward(15)
            t.left(135)
            t.forward(20)
            t.left(135)
            t.forward(15)
            t.left(180)
            t.forward(15)
            t.right(135)
            t.forward(10)
            t.left(90)
            t.forward(40)

            #Traslado
            t.penup()
            t.forward(25)
            t.right(-a1)
            t.right(90)
            t.forward(25)

            # Flecha Fuerza (P)
            t.pencolor('gold')
            t.pendown()
            t.forward(40)
            t.left(90)
            t.forward(10)
            t.right(135)
            t.forward(15)
            t.right(180)
            t.forward(15)
            t.left(135)
            t.forward(20)
            t.left(135)
            t.forward(15)
            t.left(180)
            t.forward(15)
            t.right(135)
            t.forward(10)
            t.left(90)
            t.forward(40)

            #Traslado
            t.penup()
            t.forward(25)
            t.left(90)
            t.left(-a1)

            # Cuerda
            t.penup()
            t.forward(25)
            t.left(90)
            t.forward(20)
            t.right(90)
            t.forward((l1 / 2) + 20)
            t.right(-a1)
            t.left(180)
            t.forward(10)
            t.left(180)
            t.forward(10)
            t.left(45)
            t.forward(10)
            t.right(90)
            t.forward(10)
            t.left(45)
            t.pendown()
            t.pencolor('violet')
            t.right(-a2)
            t.left(180)
            t.backward((l2 / 2) + 20)

            # Massa1
            t.pencolor('black')
            t.left(90)
            t.forward(27)
            t.backward(50)
            t.left(90)
            t.forward(50)
            t.right(90)
            t.forward(50)
            t.right(90)
            t.forward(50)
            t.right(90)
            t.forward(27)
            t.penup()
            t.right(90)
            t.forward(25)
            t.pendown()
            t.forward(1)
            t.backward(1)
            t.penup()
            t.forward(25)

            # Linea de ejes M1
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
                    
            # Letra Y del eje
            t.penup()
            t.forward(5)
            t.pendown()
            t.right(45)
            t.forward(10)
            t.left(90)
            t.forward(10)
            t.left(180)
            t.forward(20)
            t.right(180)
            t.forward(10)
            t.right(90)
            t.forward(10)
            t.left(180)
            t.forward(20)
            t.left(45)
            t.penup()
            t.forward(5)
            t.pendown()
                    
            # Linea de ejes M1
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.forward(25)
            t.left(90)
            t.forward(25)
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
                    
            # Letra Y del eje
            t.penup()
            t.forward(5)
            t.pendown()
            t.right(45)
            t.forward(20)
            t.left(180)
            t.forward(10)
            t.right(90)
            t.forward(10)
            t.right(180)
            t.forward(10)
            t.right(90)
            t.forward(10)
            t.left(45)
            t.penup()
            t.forward(5)
            t.pendown()
                    
            # Linea de ejes M1
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.forward(50)
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.left(180)
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(30)

            # Flecha Fuerza (N2)
            t.pencolor('blue')
            t.forward(25)
            t.pendown()
            t.forward(40)
            t.left(90)
            t.forward(10)
            t.right(135)
            t.forward(15)
            t.right(180)
            t.forward(15)
            t.left(135)
            t.forward(20)
            t.left(135)
            t.forward(15)
            t.left(180)
            t.forward(15)
            t.right(135)
            t.forward(10)
            t.left(90)
            t.forward(40)

            #Traslado2
            t.penup()
            t.forward(25)
            t.right(90)
            t.forward(25)

            # Flecha Fuerza (Fr2)
            t.pencolor('red')
            t.pendown()
            t.forward(40)
            t.left(90)
            t.forward(10)
            t.right(135)
            t.forward(15)
            t.right(180)
            t.forward(15)
            t.left(135)
            t.forward(20)
            t.left(135)
            t.forward(15)
            t.left(180)
            t.forward(15)
            t.right(135)
            t.forward(10)
            t.left(90)
            t.forward(40)

            #Traslado2
            t.penup()
            t.forward(25)
            t.right(a2)
            t.right(90)
            t.forward(25)

            # Flecha Fuerza (P2)
            t.pencolor('gold')
            t.pendown()
            t.forward(40)
            t.left(90)
            t.forward(10)
            t.right(135)
            t.forward(15)
            t.right(180)
            t.forward(15)
            t.left(135)
            t.forward(20)
            t.left(135)
            t.forward(15)
            t.left(180)
            t.forward(15)
            t.right(135)
            t.forward(10)
            t.left(90)
            t.forward(40)

            #Traslado2
            t.penup()
            t.forward(25)
            t.left(90)
            t.left(a2)

            # Polea
            t.right(180)
            t.forward(25)
            t.forward((l2 / 2) + 20)
            t.right(a2)
            t.right(45)
            t.forward(10)
            t.right(135)
            t.forward(10)
            t.right(90)
            t.pendown()
            t.pencolor('black')
            t.circle(-10)


            # Cuadro de leyendass
            t.pensize(3)
            t.pencolor('black')

            t.penup()
            t.setpos(-150, 25)
            t.pendown()

            # Quadrado
            t.settiltangle(0)
            t.forward(50)

            # Primera linea divisoria
            t.left(90)
            t.forward(30)

            # Letra F
            t.pencolor('lightgreen')
            t.right(90)
            t.forward(30)
            t.right(90)
            t.forward(15)
            t.right(180)
            t.forward(15)
            t.left(90)
            t.forward(10)
            t.left(90)
            t.forward(10)
            t.left(180)
            t.forward(10)
            t.left(90)
            t.forward(20)

            # Primera linea divisoria
            t.pencolor('black')
            t.left(270)
            t.forward(55)

            # Letra F de Fr
            t.pencolor('red')
            t.right(90)
            t.forward(30)
            t.right(90)
            t.forward(15)
            t.right(180)
            t.forward(15)
            t.left(90)
            t.forward(10)
            t.left(90)
            t.forward(10)
            t.left(180)
            t.forward(10)
            t.left(90)
            t.forward(20)

            # Primera linea divisoria
            t.pencolor('black')
            t.left(270)
            t.forward(15)
            t.left(180)
            t.forward(25)

            # Letra R de Fr
            t.pencolor('red')
            t.left(90)
            t.forward(16)
            t.left(180)
            t.forward(4)
            t.left(90)
            t.forward(7)
            t.right(90)
            t.forward(3)
            t.left(180)
            t.forward(3)
            t.left(90)
            t.forward(7)
            t.left(90)
            t.forward(12)

            # Primera linea divisoria
            t.pencolor('black')
            t.left(90)
            t.forward(75)

            # Quadrado
            t.left(90)
            t.forward(50)
            t.left(90)
            t.forward(100)
            t.left(90)
            t.forward(100)
            t.left(90)
            t.forward(15)

            # Letra N
            t.pencolor('blue')
            t.left(90)
            t.forward(30)
            t.left(215)
            t.forward(37)
            t.right(215)
            t.forward(30)
            t.right(180)
            t.forward(30)
            t.left(215)
            t.forward(37)
            t.right(215)
            t.forward(30)

            # Quadrado
            t.pencolor('black')
            t.left(90)
            t.forward(35)

            # Segunda linea divisoria
            t.left(90)
            t.forward(100)
            t.left(180)
            t.forward(100)

            # Quadrado
            t.left(90)
            t.forward(20)

            # Letra P
            t.pencolor('Gold')
            t.left(90)
            t.forward(30)
            t.right(90)
            t.forward(15)
            t.right(90)
            t.forward(15)
            t.right(90)
            t.forward(15)
            t.left(90)
            t.forward(15)

            # Quadrado
            t.pencolor('black')
            t.left(90)
            t.forward(30)
            t.left(90)
            t.forward(150)
            t.left(90)
            t.forward(100)
            t.left(90)
            t.forward(50)
            t.left(90)
            t.forward(50)

            # Letra T
            t.left(90)
            t.forward(2)
            t.pencolor('violet')
            t.forward(35)
            t.left(90)
            t.forward(20)
            t.left(180)
            t.forward(40)

            # cual es cada massa
            t.pencolor('black')
            x = 170
            y = 160
            t.penup()
            t.goto(x, y)
            t.write(text1, align="center", font=('Courier New', 10))

            x = -80
            y = 160
            t.goto(x, y)
            t.write(text2, align="center", font=('Courier New', 10))

            # Esconder el lapiz
            t.penup()
            t.setpos(-320, -280)
            t.pendown()
            t.forward(1)
            t.hideturtle()
            
        elif xy >= 90:

            t.speed(100)

            a1 = float(angulo_de_inclinacion1.get())
            a2 = float(angulo_de_inclinacion2.get())
            a3 = 360 - (a1 + a2)

            l1 = 300
            l2 = (math.sin(math.radians(a1)) * 300) / math.sin(math.radians(a2))
            l3 = (math.sin(math.radians(a3)) * 300) / math.sin(math.radians(a2))

            #Triangulo de dos massas
            t.pensize(4)
            t.pencolor('grey')

            t.penup()
            t.setpos(210, -55)
            t.pendown()

            t.left(-a1)
            t.backward(l1)
            t.right(a3)
            t.backward(l2)
            t.right(a2)
            t.backward(l3)

            # Polea
            t.left(-a1)
            t.backward(l1)
            t.left(a1)
            t.pencolor('black')
            t.left(90)
            t.forward(10)
            t.penup()
            t.forward(10)
            t.pendown()
            t.forward(1)
            t.backward(1)
            t.left(45)
            t.penup()
            t.forward(10)
            t.pendown()
            t.left(90)
            t.circle(10, 270)

            # Cuerda
            t.pencolor('violet')
            t.right(45)
            t.penup()
            t.forward(10)
            t.left(180)
            t.forward(10)
            t.pendown()
            t.left(-90)
            t.right(a1)
            t.backward((l1 / 2) + 20)

            # Massa2
            t.pencolor('black')
            t.left(90)
            t.forward(25)
            t.backward(50)
            t.left(90)
            t.forward(50)
            t.right(90)
            t.forward(50)
            t.right(90)
            t.forward(50)
            t.right(90)
            t.forward(25)
            t.penup()
            t.right(90)
            t.forward(25)
            t.pendown()
            t.forward(1)
            t.backward(1)
            t.penup()
            t.forward(25)

            # Linea de ejes M2
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)

            # Letra X del eje
            t.penup()
            t.forward(5)
            t.pendown()
            t.left(45)
            t.forward(10)
            t.left(90)
            t.forward(10)
            t.left(180)
            t.forward(20)
            t.right(180)
            t.forward(10)
            t.right(90)
            t.forward(10)
            t.left(180)
            t.forward(20)
            t.right(45)
            t.penup()
            t.forward(5)
            t.pendown()

            # Linea de ejes M2
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.forward(25)
            t.left(90)
            t.forward(25)
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.left(180)
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.forward(50)
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)

            # Letra Y del eje
            t.penup()
            t.forward(5)
            t.pendown()
            t.right(45)
            t.forward(20)
            t.left(180)
            t.forward(10)
            t.right(90)
            t.forward(10)
            t.right(180)
            t.forward(10)
            t.right(90)
            t.forward(10)
            t.left(45)
            t.penup()
            t.forward(5)
            t.pendown()                    
                    
            # Linea de ejes M2
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(30)

            # Flecha Fuerza (N)
            t.pencolor('blue')
            t.left(180)
            t.forward(25)
            t.pendown()
            t.forward(40)
            t.left(90)
            t.forward(10)
            t.right(135)
            t.forward(15)
            t.right(180)
            t.forward(15)
            t.left(135)
            t.forward(20)
            t.left(135)
            t.forward(15)
            t.left(180)
            t.forward(15)
            t.right(135)
            t.forward(10)
            t.left(90)
            t.forward(40)

            #Traslado
            t.penup()
            t.forward(25)
            t.right(90)
            t.forward(25)

            # Flecha Fuerza (Fr)
            t.pencolor('red')
            t.pendown()
            t.forward(40)
            t.left(90)
            t.forward(10)
            t.right(135)
            t.forward(15)
            t.right(180)
            t.forward(15)
            t.left(135)
            t.forward(20)
            t.left(135)
            t.forward(15)
            t.left(180)
            t.forward(15)
            t.right(135)
            t.forward(10)
            t.left(90)
            t.forward(40)

            #Traslado
            t.penup()
            t.forward(25)
            t.right(-a1)
            t.right(90)
            t.forward(25)

            # Flecha Fuerza (P)
            t.pencolor('gold')
            t.pendown()
            t.forward(40)
            t.left(90)
            t.forward(10)
            t.right(135)
            t.forward(15)
            t.right(180)
            t.forward(15)
            t.left(135)
            t.forward(20)
            t.left(135)
            t.forward(15)
            t.left(180)
            t.forward(15)
            t.right(135)
            t.forward(10)
            t.left(90)
            t.forward(40)

            #Traslado
            t.penup()
            t.forward(25)
            t.left(90)
            t.left(-a1)

            # Cuerda
            t.penup()
            t.forward(25)
            t.forward((l1 / 2) + 20)
            t.right(-a1)
            t.left(180)
            t.forward(10)
            t.left(180)
            t.forward(10)
            t.left(45)
            t.forward(10)
            t.right(90)
            t.forward(10)
            t.left(45)
            t.pendown()
            t.pencolor('violet')
            t.right(-a2)
            t.left(180)
            t.backward((l2 / 2) + 20)

            # Massa1
            t.pencolor('black')
            t.left(90)
            t.forward(45)
            t.backward(50)
            t.left(90)
            t.forward(50)
            t.right(90)
            t.forward(50)
            t.right(90)
            t.forward(50)
            t.right(90)
            t.forward(25)
            t.penup()
            t.right(90)
            t.forward(25)
            t.pendown()
            t.forward(1)
            t.backward(1)
            t.penup()
            t.forward(25)

            # Linea de ejes M1
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
                    
            # Letra Y del eje
            t.penup()
            t.forward(5)
            t.pendown()
            t.left(45)
            t.forward(10)
            t.left(90)
            t.forward(10)
            t.left(180)
            t.forward(20)
            t.right(180)
            t.forward(10)
            t.left(90)
            t.forward(10)
            t.right(45)
            t.penup()
            t.forward(5)
            t.pendown()
                    
            # Linea de ejes M1
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.forward(25)
            t.left(90)
            t.forward(25)
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
                    
            # Letra X del eje
            t.penup()
            t.forward(5)
            t.pendown()
            t.right(45)
            t.forward(10)
            t.left(90)
            t.forward(10)
            t.left(180)
            t.forward(20)
            t.right(180)
            t.forward(10)
            t.right(90)
            t.forward(10)
            t.left(180)
            t.forward(20)
            t.left(45)
            t.penup()
            t.forward(5)
            t.pendown()
                    
            # Linea de ejes M1
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.forward(50)
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.left(180)
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(30)

            # Flecha Fuerza (N2)
            t.pencolor('blue')
            t.forward(25)
            t.pendown()
            t.forward(40)
            t.left(90)
            t.forward(10)
            t.right(135)
            t.forward(15)
            t.right(180)
            t.forward(15)
            t.left(135)
            t.forward(20)
            t.left(135)
            t.forward(15)
            t.left(180)
            t.forward(15)
            t.right(135)
            t.forward(10)
            t.left(90)
            t.forward(40)

            #Traslado2
            t.penup()
            t.forward(25)
            t.right(90)
            t.forward(25)

            # Flecha Fuerza (Fr2)
            t.pencolor('red')
            t.pendown()
            t.forward(40)
            t.left(90)
            t.forward(10)
            t.right(135)
            t.forward(15)
            t.right(180)
            t.forward(15)
            t.left(135)
            t.forward(20)
            t.left(135)
            t.forward(15)
            t.left(180)
            t.forward(15)
            t.right(135)
            t.forward(10)
            t.left(90)
            t.forward(40)

            #Traslado2
            t.penup()
            t.forward(25)
            t.right(a2)
            t.right(90)
            t.forward(25)

            # Flecha Fuerza (P2)
            t.pencolor('gold')
            t.pendown()
            t.forward(40)
            t.left(90)
            t.forward(10)
            t.right(135)
            t.forward(15)
            t.right(180)
            t.forward(15)
            t.left(135)
            t.forward(20)
            t.left(135)
            t.forward(15)
            t.left(180)
            t.forward(15)
            t.right(135)
            t.forward(10)
            t.left(90)
            t.forward(40)

            #Traslado2
            t.penup()
            t.forward(25)
            t.left(90)
            t.left(a2)

            # Polea
            t.right(180)
            t.forward(25)
            t.right(90)
            t.forward(20)
            t.left(90)
            t.forward((l2 / 2) + 20)
            t.right(a2)
            t.right(45)
            t.forward(10)
            t.right(135)
            t.forward(10)
            t.right(90)
            t.pendown()
            t.pencolor('black')
            t.circle(-10)


            # Cuadro de leyendass
            t.pensize(3)
            t.pencolor('black')

            t.penup()
            t.setpos(-150, 25)
            t.pendown()

            # Quadrado
            t.settiltangle(0)
            t.forward(50)

            # Primera linea divisoria
            t.left(90)
            t.forward(30)

            # Letra F
            t.pencolor('lightgreen')
            t.right(90)
            t.forward(30)
            t.right(90)
            t.forward(15)
            t.right(180)
            t.forward(15)
            t.left(90)
            t.forward(10)
            t.left(90)
            t.forward(10)
            t.left(180)
            t.forward(10)
            t.left(90)
            t.forward(20)

            # Primera linea divisoria
            t.pencolor('black')
            t.left(270)
            t.forward(55)

            # Letra F de Fr
            t.pencolor('red')
            t.right(90)
            t.forward(30)
            t.right(90)
            t.forward(15)
            t.right(180)
            t.forward(15)
            t.left(90)
            t.forward(10)
            t.left(90)
            t.forward(10)
            t.left(180)
            t.forward(10)
            t.left(90)
            t.forward(20)

            # Primera linea divisoria
            t.pencolor('black')
            t.left(270)
            t.forward(15)
            t.left(180)
            t.forward(25)

            # Letra R de Fr
            t.pencolor('red')
            t.left(90)
            t.forward(16)
            t.left(180)
            t.forward(4)
            t.left(90)
            t.forward(7)
            t.right(90)
            t.forward(3)
            t.left(180)
            t.forward(3)
            t.left(90)
            t.forward(7)
            t.left(90)
            t.forward(12)

            # Primera linea divisoria
            t.pencolor('black')
            t.left(90)
            t.forward(75)

            # Quadrado
            t.left(90)
            t.forward(50)
            t.left(90)
            t.forward(100)
            t.left(90)
            t.forward(100)
            t.left(90)
            t.forward(15)

            # Letra N
            t.pencolor('blue')
            t.left(90)
            t.forward(30)
            t.left(215)
            t.forward(37)
            t.right(215)
            t.forward(30)
            t.right(180)
            t.forward(30)
            t.left(215)
            t.forward(37)
            t.right(215)
            t.forward(30)

            # Quadrado
            t.pencolor('black')
            t.left(90)
            t.forward(35)

            # Segunda linea divisoria
            t.left(90)
            t.forward(100)
            t.left(180)
            t.forward(100)

            # Quadrado
            t.left(90)
            t.forward(20)

            # Letra P
            t.pencolor('Gold')
            t.left(90)
            t.forward(30)
            t.right(90)
            t.forward(15)
            t.right(90)
            t.forward(15)
            t.right(90)
            t.forward(15)
            t.left(90)
            t.forward(15)

            # Quadrado
            t.pencolor('black')
            t.left(90)
            t.forward(30)
            t.left(90)
            t.forward(150)
            t.left(90)
            t.forward(100)
            t.left(90)
            t.forward(50)
            t.left(90)
            t.forward(50)

            # Letra T
            t.left(90)
            t.forward(2)
            t.pencolor('violet')
            t.forward(35)
            t.left(90)
            t.forward(20)
            t.left(180)
            t.forward(40)

            # cual es cada massa
            t.pencolor('black')
            x = 170
            y = 160
            t.penup()
            t.goto(x, y)
            t.write(text1, align="center", font=('Courier New', 10))

            x = -80
            y = 160
            t.goto(x, y)
            t.write(text2, align="center", font=('Courier New', 10))

            # Esconder el lapiz
            t.penup()
            t.setpos(-320, -280)
            t.pendown()
            t.forward(1)
            t.hideturtle()

        else:
            t.speed(100)

            a1 = float(angulo_de_inclinacion1.get())
            a2 = float(angulo_de_inclinacion2.get())
            a3 = 360 - (a1 + a2)

            l1 = (math.sin(math.radians(a2)) * 350) / math.sin(math.radians(a3))
            l2 = (math.sin(math.radians(a1)) * 350) / math.sin(math.radians(a3))
            l3 = 350

            #Triangulo de dos massas
            t.pensize(4)
            t.pencolor('grey')

            t.penup()
            t.setpos(220, -120)
            t.pendown()

            t.left(-a1)
            t.forward(l1)
            t.right(a3)
            t.forward(l2)
            t.right(a2)
            t.forward(l3)

            # Polea
            t.left(-a1)
            t.forward(l1)
            t.left(a1)
            t.pencolor('black')
            t.left(90)
            t.forward(10)
            t.penup()
            t.forward(10)
            t.pendown()
            t.forward(1)
            t.backward(1)
            t.left(45)
            t.penup()
            t.forward(10)
            t.pendown()
            t.left(90)
            t.circle(10, 270)

            # Cuerda
            t.pencolor('violet')
            t.right(45)
            t.penup()
            t.forward(10)
            t.left(180)
            t.forward(10)
            t.pendown()
            t.left(-90)
            t.right(a1)
            t.forward((l1 / 2) + 20)

            # Massa2
            t.pencolor('black')
            t.left(90)
            t.forward(25)
            t.backward(50)
            t.left(90)
            t.forward(50)
            t.right(90)
            t.forward(50)
            t.right(90)
            t.forward(50)
            t.right(90)
            t.forward(25)
            t.penup()
            t.right(90)
            t.forward(25)
            t.pendown()
            t.forward(1)
            t.backward(1)
            t.penup()
            t.forward(25)

            # Linea de ejes M2
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
                    
            # Letra X del eje
            t.penup()
            t.forward(5)
            t.pendown()
            t.left(45)
            t.forward(10)
            t.left(90)
            t.forward(10)
            t.left(180)
            t.forward(20)
            t.right(180)
            t.forward(10)
            t.right(90)
            t.forward(10)
            t.left(180)
            t.forward(20)
            t.right(45)
            t.penup()
            t.forward(5)
            t.pendown()

            # Linea de ejes M2
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.forward(25)
            t.left(90)
            t.forward(25)
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.left(180)
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.forward(50)
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
                    
            # Letra Y del eje
            t.penup()
            t.forward(5)
            t.pendown()
            t.right(45)
            t.forward(20)
            t.left(180)
            t.forward(10)
            t.right(90)
            t.forward(10)
            t.right(180)
            t.forward(10)
            t.right(90)
            t.forward(10)
            t.left(45)
            t.penup()
            t.forward(5)
            t.pendown()                    
                    
            # Linea de ejes M2
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(30)

            # Flecha Fuerza (N)
            t.pencolor('blue')
            t.left(180)
            t.forward(25)
            t.pendown()
            t.forward(40)
            t.left(90)
            t.forward(10)
            t.right(135)
            t.forward(15)
            t.right(180)
            t.forward(15)
            t.left(135)
            t.forward(20)
            t.left(135)
            t.forward(15)
            t.left(180)
            t.forward(15)
            t.right(135)
            t.forward(10)
            t.left(90)
            t.forward(40)

            #Traslado
            t.penup()
            t.forward(25)
            t.right(90)
            t.forward(25)

            # Flecha Fuerza (Fr)
            t.pencolor('red')
            t.pendown()
            t.forward(40)
            t.left(90)
            t.forward(10)
            t.right(135)
            t.forward(15)
            t.right(180)
            t.forward(15)
            t.left(135)
            t.forward(20)
            t.left(135)
            t.forward(15)
            t.left(180)
            t.forward(15)
            t.right(135)
            t.forward(10)
            t.left(90)
            t.forward(40)

            #Traslado
            t.penup()
            t.forward(25)
            t.right(-a1)
            t.right(90)
            t.forward(25)

            # Flecha Fuerza (P)
            t.pencolor('gold')
            t.pendown()
            t.forward(40)
            t.left(90)
            t.forward(10)
            t.right(135)
            t.forward(15)
            t.right(180)
            t.forward(15)
            t.left(135)
            t.forward(20)
            t.left(135)
            t.forward(15)
            t.left(180)
            t.forward(15)
            t.right(135)
            t.forward(10)
            t.left(90)
            t.forward(40)

            #Traslado
            t.penup()
            t.forward(25)
            t.left(90)
            t.left(-a1)
            t.left(180)

            # Cuerda
            t.penup()
            t.backward(25)
            t.forward((l1 / 2) + 20)
            t.right(-a1)
            t.forward(10)
            t.left(180)
            t.forward(10)
            t.left(45)
            t.forward(10)
            t.right(90)
            t.forward(10)
            t.left(45)
            t.pendown()
            t.pencolor('violet')
            t.right(-a2)
            t.left(180)
            t.forward((l2 / 2) + 20)

            # Massa1
            t.pencolor('black')
            t.left(90)
            t.forward(27)
            t.backward(50)
            t.left(90)
            t.forward(50)
            t.right(90)
            t.forward(50)
            t.right(90)
            t.forward(50)
            t.right(90)
            t.forward(27)
            t.penup()
            t.right(90)
            t.forward(25)
            t.pendown()
            t.forward(1)
            t.backward(1)
            t.penup()
            t.forward(25)

            # Linea de ejes M1
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
                    
            # Letra X del eje
            t.penup()
            t.forward(5)
            t.pendown()
            t.right(45)
            t.forward(10)
            t.left(90)
            t.forward(10)
            t.left(180)
            t.forward(20)
            t.right(180)
            t.forward(10)
            t.right(90)
            t.forward(10)
            t.left(180)
            t.forward(20)
            t.left(45)
            t.penup()
            t.forward(5)
            t.pendown()

            # Linea de ejes M1
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.forward(25)
            t.left(90)
            t.forward(25)
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
                    
            # Letra Y del eje
            t.penup()
            t.forward(5)
            t.pendown()
            t.right(45)
            t.forward(20)
            t.left(180)
            t.forward(10)
            t.right(90)
            t.forward(10)
            t.right(180)
            t.forward(10)
            t.right(90)
            t.forward(10)
            t.left(45)
            t.penup()
            t.forward(5)
            t.pendown()                    
                    
            # Linea de ejes M1
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.forward(50)
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.left(180)
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(30)

            # Flecha Fuerza (N2)
            t.pencolor('blue')
            t.forward(25)
            t.pendown()
            t.forward(40)
            t.left(90)
            t.forward(10)
            t.right(135)
            t.forward(15)
            t.right(180)
            t.forward(15)
            t.left(135)
            t.forward(20)
            t.left(135)
            t.forward(15)
            t.left(180)
            t.forward(15)
            t.right(135)
            t.forward(10)
            t.left(90)
            t.forward(40)

            #Traslado2
            t.penup()
            t.forward(25)
            t.right(90)
            t.forward(25)

            # Flecha Fuerza (Fr2)
            t.pencolor('red')
            t.pendown()
            t.forward(40)
            t.left(90)
            t.forward(10)
            t.right(135)
            t.forward(15)
            t.right(180)
            t.forward(15)
            t.left(135)
            t.forward(20)
            t.left(135)
            t.forward(15)
            t.left(180)
            t.forward(15)
            t.right(135)
            t.forward(10)
            t.left(90)
            t.forward(40)

            #Traslado2
            t.penup()
            t.forward(25)
            t.right(a2)
            t.right(90)
            t.forward(25)

            # Flecha Fuerza (P2)
            t.pencolor('gold')
            t.pendown()
            t.forward(40)
            t.left(90)
            t.forward(10)
            t.right(135)
            t.forward(15)
            t.right(180)
            t.forward(15)
            t.left(135)
            t.forward(20)
            t.left(135)
            t.forward(15)
            t.left(180)
            t.forward(15)
            t.right(135)
            t.forward(10)
            t.left(90)
            t.forward(40)

            #Traslado2
            t.penup()
            t.forward(25)
            t.left(90)
            t.left(a2)

            # Polea
            t.backward(25)
            t.forward((l2 / 2) + 20)
            t.right(a2)
            t.right(180)
            t.right(45)
            t.forward(10)
            t.right(135)
            t.forward(10)
            t.right(90)
            t.pendown()
            t.pencolor('black')
            t.circle(-10)


            # Cuadro de leyendass
            t.pensize(3)
            t.pencolor('black')

            t.penup()
            t.setpos(-150, 25)
            t.pendown()

            # Quadrado
            t.settiltangle(0)
            t.forward(50)

            # Primera linea divisoria
            t.left(90)
            t.forward(30)

            # Letra F
            t.pencolor('lightgreen')
            t.right(90)
            t.forward(30)
            t.right(90)
            t.forward(15)
            t.right(180)
            t.forward(15)
            t.left(90)
            t.forward(10)
            t.left(90)
            t.forward(10)
            t.left(180)
            t.forward(10)
            t.left(90)
            t.forward(20)

            # Primera linea divisoria
            t.pencolor('black')
            t.left(270)
            t.forward(55)

            # Letra F de Fr
            t.pencolor('red')
            t.right(90)
            t.forward(30)
            t.right(90)
            t.forward(15)
            t.right(180)
            t.forward(15)
            t.left(90)
            t.forward(10)
            t.left(90)
            t.forward(10)
            t.left(180)
            t.forward(10)
            t.left(90)
            t.forward(20)

            # Primera linea divisoria
            t.pencolor('black')
            t.left(270)
            t.forward(15)
            t.left(180)
            t.forward(25)

            # Letra R de Fr
            t.pencolor('red')
            t.left(90)
            t.forward(16)
            t.left(180)
            t.forward(4)
            t.left(90)
            t.forward(7)
            t.right(90)
            t.forward(3)
            t.left(180)
            t.forward(3)
            t.left(90)
            t.forward(7)
            t.left(90)
            t.forward(12)

            # Primera linea divisoria
            t.pencolor('black')
            t.left(90)
            t.forward(75)

            # Quadrado
            t.left(90)
            t.forward(50)
            t.left(90)
            t.forward(100)
            t.left(90)
            t.forward(100)
            t.left(90)
            t.forward(15)

            # Letra N
            t.pencolor('blue')
            t.left(90)
            t.forward(30)
            t.left(215)
            t.forward(37)
            t.right(215)
            t.forward(30)
            t.right(180)
            t.forward(30)
            t.left(215)
            t.forward(37)
            t.right(215)
            t.forward(30)

            # Quadrado
            t.pencolor('black')
            t.left(90)
            t.forward(35)

            # Segunda linea divisoria
            t.left(90)
            t.forward(100)
            t.left(180)
            t.forward(100)

            # Quadrado
            t.left(90)
            t.forward(20)

            # Letra P
            t.pencolor('Gold')
            t.left(90)
            t.forward(30)
            t.right(90)
            t.forward(15)
            t.right(90)
            t.forward(15)
            t.right(90)
            t.forward(15)
            t.left(90)
            t.forward(15)

            # Quadrado
            t.pencolor('black')
            t.left(90)
            t.forward(30)
            t.left(90)
            t.forward(150)
            t.left(90)
            t.forward(100)
            t.left(90)
            t.forward(50)
            t.left(90)
            t.forward(50)

            # Letra T
            t.left(90)
            t.forward(2)
            t.pencolor('violet')
            t.forward(35)
            t.left(90)
            t.forward(20)
            t.left(180)
            t.forward(40)

            # cual es cada massa
            t.pencolor('black')
            x = 170
            y = 130
            t.penup()
            t.goto(x, y)
            t.write(text1, align="center", font=('Courier New', 10))

            x = -80
            y = 130
            t.goto(x, y)
            t.write(text2, align="center", font=('Courier New', 10))

            # Esconder el lapiz
            t.penup()
            t.setpos(-320, -280)
            t.pendown()
            t.forward(1)
            t.hideturtle()

    def pop1():

        texto1 = 'Masa 1'
        texto2 = 'Masa 2'
        xz1 = float(angulo_de_inclinacion1.get())
        xy1 = float(angulo_de_inclinacion2.get())
        lienzo_turtle = turtle.ScrolledCanvas(ventana2)
        lienzo_turtle.grid(column=3, row=7, columnspan=4, rowspan=7, padx=10, pady=5)
        # Crear un objeto Turtle en el lienzo
        t = turtle.RawTurtle(lienzo_turtle)

        if xz1 >= 90 and xy1 >= 31:

            t.speed(100)

            a1 = float(angulo_de_inclinacion1.get())
            a2 = float(angulo_de_inclinacion2.get())
            a3 = 360 - (a1 + a2)

            l1 = 200
            l2 = (math.sin(math.radians(a1)) * 200) / math.sin(math.radians(a2))
            l3 = (math.sin(math.radians(a3)) * 200) / math.sin(math.radians(a2))

            #Triangulo de dos massas
            t.pensize(4)
            t.pencolor('grey')

            t.penup()
            t.setpos(150, -110)
            t.pendown()

            t.left(-a1)
            t.backward(l1)
            t.right(a3)
            t.backward(l2)
            t.right(a2)
            t.backward(l3)

            # Polea
            t.left(-a1)
            t.left(180)
            t.forward(l1)
            t.left(a1)
            t.pencolor('black')
            t.left(270)
            t.forward(10)
            t.penup()
            t.forward(10)
            t.pendown()
            t.forward(1)
            t.backward(1)
            t.left(45)
            t.penup()
            t.forward(10)
            t.pendown()
            t.left(90)
            t.circle(10, 270)

            # Cuerda
            t.pencolor('violet')
            t.right(45)
            t.penup()
            t.forward(10)
            t.left(180)
            t.forward(10)
            t.pendown()
            t.left(-90)
            t.right(a1)
            t.backward((l1 / 2) + 20)

            # Massa2
            t.pencolor('black')
            t.left(90)
            t.forward(5)
            t.backward(50)
            t.left(90)
            t.forward(50)
            t.right(90)
            t.forward(50)
            t.right(90)
            t.forward(50)
            t.right(90)
            t.forward(25)
            t.penup()
            t.right(90)
            t.forward(25)
            t.pendown()
            t.forward(1)
            t.backward(1)
            t.penup()
            t.forward(25)

            # Linea de ejes M2
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
                    
            # Letra Y del eje
            t.penup()
            t.forward(5)
            t.pendown()
            t.left(45)
            t.forward(10)
            t.left(90)
            t.forward(10)
            t.left(180)
            t.forward(20)
            t.right(180)
            t.forward(10)
            t.left(90)
            t.forward(10)
            t.right(45)
            t.penup()
            t.forward(5)
            t.pendown()
                    
            # Linea de ejes M2
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.forward(25)
            t.left(90)
            t.forward(25)
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.left(180)
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.forward(50)
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
                    
            # Letra X del eje
            t.penup()
            t.forward(5)
            t.pendown()
            t.left(45)
            t.forward(10)
            t.left(90)
            t.forward(10)
            t.left(180)
            t.forward(20)
            t.right(180)
            t.forward(10)
            t.right(90)
            t.forward(10)
            t.left(180)
            t.forward(20)
            t.right(45)
            t.penup()
            t.forward(5)
            t.pendown()
                    
            # Linea de ejes M2
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(30)

            # Flecha Fuerza (N)
            t.pencolor('blue')
            t.left(180)
            t.forward(25)
            t.pendown()
            t.forward(40)
            t.left(90)
            t.forward(10)
            t.right(135)
            t.forward(15)
            t.right(180)
            t.forward(15)
            t.left(135)
            t.forward(20)
            t.left(135)
            t.forward(15)
            t.left(180)
            t.forward(15)
            t.right(135)
            t.forward(10)
            t.left(90)
            t.forward(40)

            #Traslado
            t.penup()
            t.forward(25)
            t.left(90)
            t.forward(25)

            # Flecha Fuerza (Fr)
            t.pencolor('red')
            t.pendown()
            t.forward(40)
            t.left(90)
            t.forward(10)
            t.right(135)
            t.forward(15)
            t.right(180)
            t.forward(15)
            t.left(135)
            t.forward(20)
            t.left(135)
            t.forward(15)
            t.left(180)
            t.forward(15)
            t.right(135)
            t.forward(10)
            t.left(90)
            t.forward(40)

            #Traslado
            t.penup()
            t.forward(25)
            t.left(a1)
            t.left(90)
            t.forward(25)

            # Flecha Fuerza (P)
            t.pencolor('gold')
            t.pendown()
            t.forward(40)
            t.left(90)
            t.forward(10)
            t.right(135)
            t.forward(15)
            t.right(180)
            t.forward(15)
            t.left(135)
            t.forward(20)
            t.left(135)
            t.forward(15)
            t.left(180)
            t.forward(15)
            t.right(135)
            t.forward(10)
            t.left(90)
            t.forward(40)

            #Traslado
            t.penup()
            t.forward(25)
            t.right(90)
            t.right(a1)

            # Cuerda
            t.penup()
            t.backward(25)
            t.right(90)
            t.forward(20)
            t.left(90)
            t.backward((l1 / 2) + 20)
            t.right(-a1)
            t.forward(10)
            t.left(180)
            t.forward(10)
            t.left(45)
            t.forward(10)
            t.right(90)
            t.forward(10)
            t.left(45)
            t.pendown()
            t.pencolor('violet')
            t.right(-a2)
            t.left(180)
            t.backward((l2 / 2) + 20)

            # Massa1
            t.pencolor('black')
            t.left(90)
            t.forward(27)
            t.backward(50)
            t.left(90)
            t.forward(50)
            t.right(90)
            t.forward(50)
            t.right(90)
            t.forward(50)
            t.right(90)
            t.forward(27)
            t.penup()
            t.right(90)
            t.forward(25)
            t.pendown()
            t.forward(1)
            t.backward(1)
            t.penup()
            t.forward(25)

            # Linea de ejes M1
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
                    
            # Letra X del eje
            t.penup()
            t.forward(5)
            t.pendown()
            t.right(45)
            t.forward(10)
            t.left(90)
            t.forward(10)
            t.left(180)
            t.forward(20)
            t.right(180)
            t.forward(10)
            t.right(90)
            t.forward(10)
            t.left(180)
            t.forward(20)
            t.left(45)
            t.penup()
            t.forward(5)
            t.pendown()

            # Linea de ejes M1
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.forward(25)
            t.left(90)
            t.forward(25)
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
                    
            # Letra Y del eje
            t.penup()
            t.forward(5)
            t.pendown()
            t.right(45)
            t.forward(20)
            t.left(180)
            t.forward(10)
            t.right(90)
            t.forward(10)
            t.right(180)
            t.forward(10)
            t.right(90)
            t.forward(10)
            t.left(45)
            t.penup()
            t.forward(5)
            t.pendown()                    
                    
            # Linea de ejes M1
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.forward(50)
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.left(180)
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(30)

            # Flecha Fuerza (N2)
            t.pencolor('blue')
            t.forward(25)
            t.pendown()
            t.forward(40)
            t.left(90)
            t.forward(10)
            t.right(135)
            t.forward(15)
            t.right(180)
            t.forward(15)
            t.left(135)
            t.forward(20)
            t.left(135)
            t.forward(15)
            t.left(180)
            t.forward(15)
            t.right(135)
            t.forward(10)
            t.left(90)
            t.forward(40)

            #Traslado2
            t.penup()
            t.forward(25)
            t.left(90)
            t.forward(25)

            # Flecha Fuerza (Fr2)
            t.pencolor('red')
            t.pendown()
            t.forward(40)
            t.left(90)
            t.forward(10)
            t.right(135)
            t.forward(15)
            t.right(180)
            t.forward(15)
            t.left(135)
            t.forward(20)
            t.left(135)
            t.forward(15)
            t.left(180)
            t.forward(15)
            t.right(135)
            t.forward(10)
            t.left(90)
            t.forward(40)

            #Traslado2
            t.penup()
            t.forward(25)
            t.left(-a2)
            t.left(90)
            t.forward(25)

            # Flecha Fuerza (P2)
            t.pencolor('gold')
            t.pendown()
            t.forward(40)
            t.left(90)
            t.forward(10)
            t.right(135)
            t.forward(15)
            t.right(180)
            t.forward(15)
            t.left(135)
            t.forward(20)
            t.left(135)
            t.forward(15)
            t.left(180)
            t.forward(15)
            t.right(135)
            t.forward(10)
            t.left(90)
            t.forward(40)

            #Traslado2
            t.penup()
            t.forward(25)
            t.right(90)
            t.right(-a2)
            t.left(180)

            # Polea
            t.backward(25)
            t.backward((l2 / 2) + 20)
            t.right(a2)
            t.right(180)
            t.right(45)
            t.forward(10)
            t.right(135)
            t.forward(10)
            t.right(90)
            t.pendown()
            t.pencolor('black')
            t.circle(-10)


            # Cuadro de leyendass
            t.pensize(3)
            t.pencolor('black')

            t.penup()
            t.setpos(-150, 25)
            t.pendown()

            # Quadrado
            t.settiltangle(0)
            t.forward(50)

            # Primera linea divisoria
            t.left(90)
            t.forward(30)

            # Letra F
            t.pencolor('lightgreen')
            t.right(90)
            t.forward(30)
            t.right(90)
            t.forward(15)
            t.right(180)
            t.forward(15)
            t.left(90)
            t.forward(10)
            t.left(90)
            t.forward(10)
            t.left(180)
            t.forward(10)
            t.left(90)
            t.forward(20)

            # Primera linea divisoria
            t.pencolor('black')
            t.left(270)
            t.forward(55)

            # Letra F de Fr
            t.pencolor('red')
            t.right(90)
            t.forward(30)
            t.right(90)
            t.forward(15)
            t.right(180)
            t.forward(15)
            t.left(90)
            t.forward(10)
            t.left(90)
            t.forward(10)
            t.left(180)
            t.forward(10)
            t.left(90)
            t.forward(20)

            # Primera linea divisoria
            t.pencolor('black')
            t.left(270)
            t.forward(15)
            t.left(180)
            t.forward(25)

            # Letra R de Fr
            t.pencolor('red')
            t.left(90)
            t.forward(16)
            t.left(180)
            t.forward(4)
            t.left(90)
            t.forward(7)
            t.right(90)
            t.forward(3)
            t.left(180)
            t.forward(3)
            t.left(90)
            t.forward(7)
            t.left(90)
            t.forward(12)

            # Primera linea divisoria
            t.pencolor('black')
            t.left(90)
            t.forward(75)

            # Quadrado
            t.left(90)
            t.forward(50)
            t.left(90)
            t.forward(100)
            t.left(90)
            t.forward(100)
            t.left(90)
            t.forward(15)

            # Letra N
            t.pencolor('blue')
            t.left(90)
            t.forward(30)
            t.left(215)
            t.forward(37)
            t.right(215)
            t.forward(30)
            t.right(180)
            t.forward(30)
            t.left(215)
            t.forward(37)
            t.right(215)
            t.forward(30)

            # Quadrado
            t.pencolor('black')
            t.left(90)
            t.forward(35)

            # Segunda linea divisoria
            t.left(90)
            t.forward(100)
            t.left(180)
            t.forward(100)

            # Quadrado
            t.left(90)
            t.forward(20)

            # Letra P
            t.pencolor('Gold')
            t.left(90)
            t.forward(30)
            t.right(90)
            t.forward(15)
            t.right(90)
            t.forward(15)
            t.right(90)
            t.forward(15)
            t.left(90)
            t.forward(15)

            # Quadrado
            t.pencolor('black')
            t.left(90)
            t.forward(30)
            t.left(90)
            t.forward(150)
            t.left(90)
            t.forward(100)
            t.left(90)
            t.forward(50)
            t.left(90)
            t.forward(50)

            # Letra T
            t.left(90)
            t.forward(2)
            t.pencolor('violet')
            t.forward(35)
            t.left(90)
            t.forward(20)
            t.left(180)
            t.forward(40)

            # cual es cada massa
            t.pencolor('black')
            x = 200
            y = 130
            t.penup()
            t.goto(x, y)
            t.write(texto1, align="center", font=('Courier New', 10))

            x = 0
            y = 130
            t.goto(x, y)
            t.write(texto2, align="center", font=('Courier New', 10))

            # Esconder el lapiz
            t.penup()
            t.setpos(-320, -280)
            t.pendown()
            t.forward(1)
            t.hideturtle()

        elif xz1 >= 31 and xy1 >= 90:
            
            t.speed(100)

            a1 = float(angulo_de_inclinacion1.get())
            a2 = float(angulo_de_inclinacion2.get())
            a3 = 360 - (a1 + a2)

            l1 = 300
            l2 = (math.sin(math.radians(a1)) * 300) / math.sin(math.radians(a2))
            l3 = (math.sin(math.radians(a3)) * 300) / math.sin(math.radians(a2))

            #Triangulo de dos massas
            t.pensize(4)
            t.pencolor('grey')

            t.penup()
            t.setpos(180, -120)
            t.pendown()

            t.left(-a1)
            t.backward(l1)
            t.right(a3)
            t.backward(l2)
            t.right(a2)
            t.backward(l3)

            # Polea
            t.left(-a1)
            t.left(180)
            t.forward(l1)
            t.left(a1)
            t.pencolor('black')
            t.left(270)
            t.forward(10)
            t.penup()
            t.forward(10)
            t.pendown()
            t.forward(1)
            t.backward(1)
            t.left(45)
            t.penup()
            t.forward(10)
            t.pendown()
            t.left(90)
            t.circle(10, 270)

            # Cuerda
            t.pencolor('violet')
            t.right(45)
            t.penup()
            t.forward(10)
            t.left(180)
            t.forward(10)
            t.pendown()
            t.left(-90)
            t.right(a1)
            t.backward((l1 / 2) + 20)

            # Massa2
            t.pencolor('black')
            t.left(90)
            t.forward(25)
            t.backward(50)
            t.left(90)
            t.forward(50)
            t.right(90)
            t.forward(50)
            t.right(90)
            t.forward(50)
            t.right(90)
            t.forward(25)
            t.penup()
            t.right(90)
            t.forward(25)
            t.pendown()
            t.forward(1)
            t.backward(1)
            t.penup()
            t.forward(25)

            # Linea de ejes M2
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
                    
            # Letra X del eje
            t.penup()
            t.forward(5)
            t.pendown()
            t.left(45)
            t.forward(10)
            t.left(90)
            t.forward(10)
            t.left(180)
            t.forward(20)
            t.right(180)
            t.forward(10)
            t.right(90)
            t.forward(10)
            t.left(180)
            t.forward(20)
            t.right(45)
            t.penup()
            t.forward(5)
            t.pendown()

            # Linea de ejes M2
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.forward(25)
            t.left(90)
            t.forward(25)
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.left(180)
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.forward(50)
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
                    
            # Letra Y del eje
            t.penup()
            t.forward(5)
            t.pendown()
            t.right(45)
            t.forward(20)
            t.left(180)
            t.forward(10)
            t.right(90)
            t.forward(10)
            t.right(180)
            t.forward(10)
            t.right(90)
            t.forward(10)
            t.left(45)
            t.penup()
            t.forward(5)
            t.pendown()                    
                    
            # Linea de ejes M2
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(30)

            # Flecha Fuerza (N)
            t.pencolor('blue')
            t.left(180)
            t.forward(25)
            t.pendown()
            t.forward(40)
            t.left(90)
            t.forward(10)
            t.right(135)
            t.forward(15)
            t.right(180)
            t.forward(15)
            t.left(135)
            t.forward(20)
            t.left(135)
            t.forward(15)
            t.left(180)
            t.forward(15)
            t.right(135)
            t.forward(10)
            t.left(90)
            t.forward(40)

            #Traslado
            t.penup()
            t.forward(25)
            t.left(90)
            t.forward(25)

            # Flecha Fuerza (Fr)
            t.pencolor('red')
            t.pendown()
            t.forward(40)
            t.left(90)
            t.forward(10)
            t.right(135)
            t.forward(15)
            t.right(180)
            t.forward(15)
            t.left(135)
            t.forward(20)
            t.left(135)
            t.forward(15)
            t.left(180)
            t.forward(15)
            t.right(135)
            t.forward(10)
            t.left(90)
            t.forward(40)

            #Traslado
            t.penup()
            t.forward(25)
            t.left(a1)
            t.left(90)
            t.forward(25)

            # Flecha Fuerza (P)
            t.pencolor('gold')
            t.pendown()
            t.forward(40)
            t.left(90)
            t.forward(10)
            t.right(135)
            t.forward(15)
            t.right(180)
            t.forward(15)
            t.left(135)
            t.forward(20)
            t.left(135)
            t.forward(15)
            t.left(180)
            t.forward(15)
            t.right(135)
            t.forward(10)
            t.left(90)
            t.forward(40)

            #Traslado
            t.penup()
            t.forward(25)
            t.right(90)
            t.right(a1)

            # Cuerda
            t.penup()
            t.backward(25)
            t.backward((l1 / 2) + 20)
            t.right(-a1)
            t.forward(10)
            t.left(180)
            t.forward(10)
            t.left(45)
            t.forward(10)
            t.right(90)
            t.forward(10)
            t.left(45)
            t.pendown()
            t.pencolor('violet')
            t.right(-a2)
            t.left(180)
            t.backward((l2 / 2) + 20)

            # Massa1
            t.pencolor('black')
            t.left(90)
            t.forward(45)
            t.backward(50)
            t.left(90)
            t.forward(50)
            t.right(90)
            t.forward(50)
            t.right(90)
            t.forward(50)
            t.right(90)
            t.forward(25)
            t.penup()
            t.right(90)
            t.forward(25)
            t.pendown()
            t.forward(1)
            t.backward(1)
            t.penup()
            t.forward(25)

            # Linea de ejes M1
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
                    
            # Letra Y del eje
            t.penup()
            t.forward(5)
            t.pendown()
            t.left(45)
            t.forward(10)
            t.left(90)
            t.forward(10)
            t.left(180)
            t.forward(20)
            t.right(180)
            t.forward(10)
            t.left(90)
            t.forward(10)
            t.right(45)
            t.penup()
            t.forward(5)
            t.pendown()
                    
            # Linea de ejes M1
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.forward(25)
            t.left(90)
            t.forward(25)
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
                    
            # Letra X del eje
            t.penup()
            t.forward(5)
            t.pendown()
            t.right(45)
            t.forward(10)
            t.left(90)
            t.forward(10)
            t.left(180)
            t.forward(20)
            t.right(180)
            t.forward(10)
            t.right(90)
            t.forward(10)
            t.left(180)
            t.forward(20)
            t.left(45)
            t.penup()
            t.forward(5)
            t.pendown()
                    
            # Linea de ejes M1
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.forward(50)
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.left(180)
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(30)

            # Flecha Fuerza (N2)
            t.pencolor('blue')
            t.forward(25)
            t.pendown()
            t.forward(40)
            t.left(90)
            t.forward(10)
            t.right(135)
            t.forward(15)
            t.right(180)
            t.forward(15)
            t.left(135)
            t.forward(20)
            t.left(135)
            t.forward(15)
            t.left(180)
            t.forward(15)
            t.right(135)
            t.forward(10)
            t.left(90)
            t.forward(40)

            #Traslado2
            t.penup()
            t.forward(25)
            t.left(90)
            t.forward(25)

            # Flecha Fuerza (Fr2)
            t.pencolor('red')
            t.pendown()
            t.forward(40)
            t.left(90)
            t.forward(10)
            t.right(135)
            t.forward(15)
            t.right(180)
            t.forward(15)
            t.left(135)
            t.forward(20)
            t.left(135)
            t.forward(15)
            t.left(180)
            t.forward(15)
            t.right(135)
            t.forward(10)
            t.left(90)
            t.forward(40)

            #Traslado2
            t.penup()
            t.forward(25)
            t.left(-a2)
            t.left(90)
            t.forward(25)

            # Flecha Fuerza (P2)
            t.pencolor('gold')
            t.pendown()
            t.forward(40)
            t.left(90)
            t.forward(10)
            t.right(135)
            t.forward(15)
            t.right(180)
            t.forward(15)
            t.left(135)
            t.forward(20)
            t.left(135)
            t.forward(15)
            t.left(180)
            t.forward(15)
            t.right(135)
            t.forward(10)
            t.left(90)
            t.forward(40)

            #Traslado2
            t.penup()
            t.forward(25)
            t.right(90)
            t.right(-a2)
            t.left(180)

            # Polea
            t.backward(25)
            t.left(90)
            t.forward(20)
            t.right(90)
            t.backward((l2 / 2) + 20)
            t.right(a2)
            t.right(180)
            t.right(45)
            t.forward(10)
            t.right(135)
            t.forward(10)
            t.right(90)
            t.pendown()
            t.pencolor('black')
            t.circle(-10)


            # Cuadro de leyendass
            t.pensize(3)
            t.pencolor('black')

            t.penup()
            t.setpos(-150, 25)
            t.pendown()

            # Quadrado
            t.settiltangle(0)
            t.forward(50)

            # Primera linea divisoria
            t.left(90)
            t.forward(30)

            # Letra F
            t.pencolor('lightgreen')
            t.right(90)
            t.forward(30)
            t.right(90)
            t.forward(15)
            t.right(180)
            t.forward(15)
            t.left(90)
            t.forward(10)
            t.left(90)
            t.forward(10)
            t.left(180)
            t.forward(10)
            t.left(90)
            t.forward(20)

            # Primera linea divisoria
            t.pencolor('black')
            t.left(270)
            t.forward(55)

            # Letra F de Fr
            t.pencolor('red')
            t.right(90)
            t.forward(30)
            t.right(90)
            t.forward(15)
            t.right(180)
            t.forward(15)
            t.left(90)
            t.forward(10)
            t.left(90)
            t.forward(10)
            t.left(180)
            t.forward(10)
            t.left(90)
            t.forward(20)

            # Primera linea divisoria
            t.pencolor('black')
            t.left(270)
            t.forward(15)
            t.left(180)
            t.forward(25)

            # Letra R de Fr
            t.pencolor('red')
            t.left(90)
            t.forward(16)
            t.left(180)
            t.forward(4)
            t.left(90)
            t.forward(7)
            t.right(90)
            t.forward(3)
            t.left(180)
            t.forward(3)
            t.left(90)
            t.forward(7)
            t.left(90)
            t.forward(12)

            # Primera linea divisoria
            t.pencolor('black')
            t.left(90)
            t.forward(75)

            # Quadrado
            t.left(90)
            t.forward(50)
            t.left(90)
            t.forward(100)
            t.left(90)
            t.forward(100)
            t.left(90)
            t.forward(15)

            # Letra N
            t.pencolor('blue')
            t.left(90)
            t.forward(30)
            t.left(215)
            t.forward(37)
            t.right(215)
            t.forward(30)
            t.right(180)
            t.forward(30)
            t.left(215)
            t.forward(37)
            t.right(215)
            t.forward(30)

            # Quadrado
            t.pencolor('black')
            t.left(90)
            t.forward(35)

            # Segunda linea divisoria
            t.left(90)
            t.forward(100)
            t.left(180)
            t.forward(100)

            # Quadrado
            t.left(90)
            t.forward(20)

            # Letra P
            t.pencolor('Gold')
            t.left(90)
            t.forward(30)
            t.right(90)
            t.forward(15)
            t.right(90)
            t.forward(15)
            t.right(90)
            t.forward(15)
            t.left(90)
            t.forward(15)

            # Quadrado
            t.pencolor('black')
            t.left(90)
            t.forward(30)
            t.left(90)
            t.forward(150)
            t.left(90)
            t.forward(100)
            t.left(90)
            t.forward(50)
            t.left(90)
            t.forward(50)

            # Letra T
            t.left(90)
            t.forward(2)
            t.pencolor('violet')
            t.forward(35)
            t.left(90)
            t.forward(20)
            t.left(180)
            t.forward(40)

            # cual es cada massa
            t.pencolor('black')
            x = 160
            y = 150
            t.penup()
            t.goto(x, y)
            t.write(texto1, align="center", font=('Courier New', 10))

            x = -60
            y = 150
            t.goto(x, y)
            t.write(texto2, align="center", font=('Courier New', 10))

            # Esconder el lapiz
            t.penup()
            t.setpos(-320, -280)
            t.pendown()
            t.forward(1)
            t.hideturtle()

        elif xz1 >= 51 and xy1 >= 51:

            t.speed(100)

            a1 = float(angulo_de_inclinacion1.get())
            a2 = float(angulo_de_inclinacion2.get())
            a3 = 360 - (a1 + a2)

            l1 = 250
            l2 = (math.sin(math.radians(a1)) * 250) / math.sin(math.radians(a2))
            l3 = (math.sin(math.radians(a3)) * 250) / math.sin(math.radians(a2))

            #Triangulo de dos massas
            t.pensize(4)
            t.pencolor('grey')

            t.penup()
            t.setpos(170, -160)
            t.pendown()

            t.left(-a1)
            t.backward(l1)
            t.right(a3)
            t.backward(l2)
            t.right(a2)
            t.backward(l3)

            # Polea
            t.left(-a1)
            t.left(180)
            t.forward(l1)
            t.left(a1)
            t.pencolor('black')
            t.left(270)
            t.forward(10)
            t.penup()
            t.forward(10)
            t.pendown()
            t.forward(1)
            t.backward(1)
            t.left(45)
            t.penup()
            t.forward(10)
            t.pendown()
            t.left(90)
            t.circle(10, 270)

            # Cuerda
            t.pencolor('violet')
            t.right(45)
            t.penup()
            t.forward(10)
            t.left(180)
            t.forward(10)
            t.pendown()
            t.left(-90)
            t.right(a1)
            t.backward((l1 / 2) + 20)

            # Massa2
            t.pencolor('black')
            t.left(90)
            t.forward(25)
            t.backward(50)
            t.left(90)
            t.forward(50)
            t.right(90)
            t.forward(50)
            t.right(90)
            t.forward(50)
            t.right(90)
            t.forward(25)
            t.penup()
            t.right(90)
            t.forward(25)
            t.pendown()
            t.forward(1)
            t.backward(1)
            t.penup()
            t.forward(25)

            # Linea de ejes M2
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
                    
            # Letra X del eje
            t.penup()
            t.forward(5)
            t.pendown()
            t.left(45)
            t.forward(10)
            t.left(90)
            t.forward(10)
            t.left(180)
            t.forward(20)
            t.right(180)
            t.forward(10)
            t.right(90)
            t.forward(10)
            t.left(180)
            t.forward(20)
            t.right(45)
            t.penup()
            t.forward(5)
            t.pendown()

            # Linea de ejes M2
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.forward(25)
            t.left(90)
            t.forward(25)
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.left(180)
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.forward(50)
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
                    
            # Letra Y del eje
            t.penup()
            t.forward(5)
            t.pendown()
            t.right(45)
            t.forward(20)
            t.left(180)
            t.forward(10)
            t.right(90)
            t.forward(10)
            t.right(180)
            t.forward(10)
            t.right(90)
            t.forward(10)
            t.left(45)
            t.penup()
            t.forward(5)
            t.pendown()                    
                    
            # Linea de ejes M2
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(30)

            # Flecha Fuerza (N)
            t.pencolor('blue')
            t.left(180)
            t.forward(25)
            t.pendown()
            t.forward(40)
            t.left(90)
            t.forward(10)
            t.right(135)
            t.forward(15)
            t.right(180)
            t.forward(15)
            t.left(135)
            t.forward(20)
            t.left(135)
            t.forward(15)
            t.left(180)
            t.forward(15)
            t.right(135)
            t.forward(10)
            t.left(90)
            t.forward(40)

            #Traslado
            t.penup()
            t.forward(25)
            t.left(90)
            t.forward(25)

            # Flecha Fuerza (Fr)
            t.pencolor('red')
            t.pendown()
            t.forward(40)
            t.left(90)
            t.forward(10)
            t.right(135)
            t.forward(15)
            t.right(180)
            t.forward(15)
            t.left(135)
            t.forward(20)
            t.left(135)
            t.forward(15)
            t.left(180)
            t.forward(15)
            t.right(135)
            t.forward(10)
            t.left(90)
            t.forward(40)

            #Traslado
            t.penup()
            t.forward(25)
            t.left(a1)
            t.left(90)
            t.forward(25)

            # Flecha Fuerza (P)
            t.pencolor('gold')
            t.pendown()
            t.forward(40)
            t.left(90)
            t.forward(10)
            t.right(135)
            t.forward(15)
            t.right(180)
            t.forward(15)
            t.left(135)
            t.forward(20)
            t.left(135)
            t.forward(15)
            t.left(180)
            t.forward(15)
            t.right(135)
            t.forward(10)
            t.left(90)
            t.forward(40)

            #Traslado
            t.penup()
            t.forward(25)
            t.right(90)
            t.right(a1)

            # Cuerda
            t.penup()
            t.backward(25)
            t.backward((l1 / 2) + 20)
            t.right(-a1)
            t.forward(10)
            t.left(180)
            t.forward(10)
            t.left(45)
            t.forward(10)
            t.right(90)
            t.forward(10)
            t.left(45)
            t.pendown()
            t.pencolor('violet')
            t.right(-a2)
            t.left(180)
            t.backward((l2 / 2) + 20)

            # Massa1
            t.pencolor('black')
            t.left(90)
            t.forward(27)
            t.backward(50)
            t.left(90)
            t.forward(50)
            t.right(90)
            t.forward(50)
            t.right(90)
            t.forward(50)
            t.right(90)
            t.forward(27)
            t.penup()
            t.right(90)
            t.forward(25)
            t.pendown()
            t.forward(1)
            t.backward(1)
            t.penup()
            t.forward(25)

            # Linea de ejes M1
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
                    
            # Letra X del eje
            t.penup()
            t.forward(5)
            t.pendown()
            t.right(45)
            t.forward(10)
            t.left(90)
            t.forward(10)
            t.left(180)
            t.forward(20)
            t.right(180)
            t.forward(10)
            t.right(90)
            t.forward(10)
            t.left(180)
            t.forward(20)
            t.left(45)
            t.penup()
            t.forward(5)
            t.pendown()

            # Linea de ejes M2
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.forward(25)
            t.left(90)
            t.forward(25)
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
        
            # Letra Y del eje
            t.penup()
            t.forward(5)
            t.pendown()
            t.right(45)
            t.forward(20)
            t.left(180)
            t.forward(10)
            t.right(90)
            t.forward(10)
            t.right(180)
            t.forward(10)
            t.right(90)
            t.forward(10)
            t.left(45)
            t.penup()
            t.forward(5)
            t.pendown()                    
                    
            # Linea de ejes M2
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.forward(50)
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.left(180)
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(30)

            # Flecha Fuerza (N2)
            t.pencolor('blue')
            t.forward(25)
            t.pendown()
            t.forward(40)
            t.left(90)
            t.forward(10)
            t.right(135)
            t.forward(15)
            t.right(180)
            t.forward(15)
            t.left(135)
            t.forward(20)
            t.left(135)
            t.forward(15)
            t.left(180)
            t.forward(15)
            t.right(135)
            t.forward(10)
            t.left(90)
            t.forward(40)

            #Traslado2
            t.penup()
            t.forward(25)
            t.left(90)
            t.forward(25)

            # Flecha Fuerza (Fr2)
            t.pencolor('red')
            t.pendown()
            t.forward(40)
            t.left(90)
            t.forward(10)
            t.right(135)
            t.forward(15)
            t.right(180)
            t.forward(15)
            t.left(135)
            t.forward(20)
            t.left(135)
            t.forward(15)
            t.left(180)
            t.forward(15)
            t.right(135)
            t.forward(10)
            t.left(90)
            t.forward(40)

            #Traslado2
            t.penup()
            t.forward(25)
            t.left(-a2)
            t.left(90)
            t.forward(25)

            # Flecha Fuerza (P2)
            t.pencolor('gold')
            t.pendown()
            t.forward(40)
            t.left(90)
            t.forward(10)
            t.right(135)
            t.forward(15)
            t.right(180)
            t.forward(15)
            t.left(135)
            t.forward(20)
            t.left(135)
            t.forward(15)
            t.left(180)
            t.forward(15)
            t.right(135)
            t.forward(10)
            t.left(90)
            t.forward(40)

            #Traslado2
            t.penup()
            t.forward(25)
            t.right(90)
            t.right(-a2)
            t.left(180)

            # Polea
            t.backward(25)
            t.backward((l2 / 2) + 20)
            t.right(a2)
            t.right(180)
            t.right(45)
            t.forward(10)
            t.right(135)
            t.forward(10)
            t.right(90)
            t.pendown()
            t.pencolor('black')
            t.circle(-10)


            # Cuadro de leyendass
            t.pensize(3)
            t.pencolor('black')

            t.penup()
            t.setpos(-150, 25)
            t.pendown()

            # Quadrado
            t.settiltangle(0)
            t.forward(50)

            # Primera linea divisoria
            t.left(90)
            t.forward(30)

            # Letra F
            t.pencolor('lightgreen')
            t.right(90)
            t.forward(30)
            t.right(90)
            t.forward(15)
            t.right(180)
            t.forward(15)
            t.left(90)
            t.forward(10)
            t.left(90)
            t.forward(10)
            t.left(180)
            t.forward(10)
            t.left(90)
            t.forward(20)

            # Primera linea divisoria
            t.pencolor('black')
            t.left(270)
            t.forward(55)

            # Letra F de Fr
            t.pencolor('red')
            t.right(90)
            t.forward(30)
            t.right(90)
            t.forward(15)
            t.right(180)
            t.forward(15)
            t.left(90)
            t.forward(10)
            t.left(90)
            t.forward(10)
            t.left(180)
            t.forward(10)
            t.left(90)
            t.forward(20)

            # Primera linea divisoria
            t.pencolor('black')
            t.left(270)
            t.forward(15)
            t.left(180)
            t.forward(25)

            # Letra R de Fr
            t.pencolor('red')
            t.left(90)
            t.forward(16)
            t.left(180)
            t.forward(4)
            t.left(90)
            t.forward(7)
            t.right(90)
            t.forward(3)
            t.left(180)
            t.forward(3)
            t.left(90)
            t.forward(7)
            t.left(90)
            t.forward(12)

            # Primera linea divisoria
            t.pencolor('black')
            t.left(90)
            t.forward(75)

            # Quadrado
            t.left(90)
            t.forward(50)
            t.left(90)
            t.forward(100)
            t.left(90)
            t.forward(100)
            t.left(90)
            t.forward(15)

            # Letra N
            t.pencolor('blue')
            t.left(90)
            t.forward(30)
            t.left(215)
            t.forward(37)
            t.right(215)
            t.forward(30)
            t.right(180)
            t.forward(30)
            t.left(215)
            t.forward(37)
            t.right(215)
            t.forward(30)

            # Quadrado
            t.pencolor('black')
            t.left(90)
            t.forward(35)

            # Segunda linea divisoria
            t.left(90)
            t.forward(100)
            t.left(180)
            t.forward(100)

            # Quadrado
            t.left(90)
            t.forward(20)

            # Letra P
            t.pencolor('Gold')
            t.left(90)
            t.forward(30)
            t.right(90)
            t.forward(15)
            t.right(90)
            t.forward(15)
            t.right(90)
            t.forward(15)
            t.left(90)
            t.forward(15)

            # Quadrado
            t.pencolor('black')
            t.left(90)
            t.forward(30)
            t.left(90)
            t.forward(150)
            t.left(90)
            t.forward(100)
            t.left(90)
            t.forward(50)
            t.left(90)
            t.forward(50)

            # Letra T
            t.left(90)
            t.forward(2)
            t.pencolor('violet')
            t.forward(35)
            t.left(90)
            t.forward(20)
            t.left(180)
            t.forward(40)

            # cual es cada massa
            t.pencolor('black')
            x = 130
            y = 80
            t.penup()
            t.goto(x, y)
            t.write(texto1, align="center", font=('Courier New', 10))

            x = -80
            y = 80
            t.goto(x, y)
            t.write(texto2, align="center", font=('Courier New', 10))

            # Esconder el lapiz
            t.penup()
            t.setpos(-320, -280)
            t.pendown()
            t.forward(1)
            t.hideturtle()

        elif xz1 >= 90:

            t.speed(100)

            a1 = float(angulo_de_inclinacion1.get())
            a2 = float(angulo_de_inclinacion2.get())
            a3 = 360 - (a1 + a2)

            l1 = 100
            l2 = (math.sin(math.radians(a1)) * 100) / math.sin(math.radians(a2))
            l3 = (math.sin(math.radians(a3)) * 100) / math.sin(math.radians(a2))

            #Triangulo de dos massas
            t.pensize(4)
            t.pencolor('grey')

            t.penup()
            t.setpos(140, -80)
            t.pendown()

            t.left(-a1)
            t.backward(l1)
            t.right(a3)
            t.backward(l2)
            t.right(a2)
            t.backward(l3)

            # Polea
            t.left(-a1)
            t.left(180)
            t.forward(l1)
            t.left(a1)
            t.pencolor('black')
            t.left(270)
            t.forward(10)
            t.penup()
            t.forward(10)
            t.pendown()
            t.forward(1)
            t.backward(1)
            t.left(45)
            t.penup()
            t.forward(10)
            t.pendown()
            t.left(90)
            t.circle(10, 270)

            # Cuerda
            t.pencolor('violet')
            t.right(45)
            t.penup()
            t.forward(10)
            t.left(180)
            t.forward(10)
            t.pendown()
            t.left(-90)
            t.right(a1)
            t.backward((l1 / 2) + 20)

            # Massa2
            t.pencolor('black')
            t.left(90)
            t.forward(5)
            t.backward(50)
            t.left(90)
            t.forward(50)
            t.right(90)
            t.forward(50)
            t.right(90)
            t.forward(50)
            t.right(90)
            t.forward(25)
            t.penup()
            t.right(90)
            t.forward(25)
            t.pendown()
            t.forward(1)
            t.backward(1)
            t.penup()
            t.forward(25)

            # Linea de ejes M2
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
                    
            # Letra Y del eje
            t.penup()
            t.forward(5)
            t.pendown()
            t.left(45)
            t.forward(10)
            t.left(90)
            t.forward(10)
            t.left(180)
            t.forward(20)
            t.right(180)
            t.forward(10)
            t.left(90)
            t.forward(10)
            t.right(45)
            t.penup()
            t.forward(5)
            t.pendown()
                    
            # Linea de ejes M2
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.forward(25)
            t.left(90)
            t.forward(25)
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.left(180)
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.forward(50)
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
                    
            # Letra X del eje
            t.penup()
            t.forward(5)
            t.pendown()
            t.left(45)
            t.forward(10)
            t.left(90)
            t.forward(10)
            t.left(180)
            t.forward(20)
            t.right(180)
            t.forward(10)
            t.right(90)
            t.forward(10)
            t.left(180)
            t.forward(20)
            t.right(45)
            t.penup()
            t.forward(5)
            t.pendown()
                    
            # Linea de ejes M2
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(30)

            # Flecha Fuerza (N)
            t.pencolor('blue')
            t.left(180)
            t.forward(25)
            t.pendown()
            t.forward(40)
            t.left(90)
            t.forward(10)
            t.right(135)
            t.forward(15)
            t.right(180)
            t.forward(15)
            t.left(135)
            t.forward(20)
            t.left(135)
            t.forward(15)
            t.left(180)
            t.forward(15)
            t.right(135)
            t.forward(10)
            t.left(90)
            t.forward(40)

            #Traslado
            t.penup()
            t.forward(25)
            t.left(90)
            t.forward(25)

            # Flecha Fuerza (Fr)
            t.pencolor('red')
            t.pendown()
            t.forward(40)
            t.left(90)
            t.forward(10)
            t.right(135)
            t.forward(15)
            t.right(180)
            t.forward(15)
            t.left(135)
            t.forward(20)
            t.left(135)
            t.forward(15)
            t.left(180)
            t.forward(15)
            t.right(135)
            t.forward(10)
            t.left(90)
            t.forward(40)

            #Traslado
            t.penup()
            t.forward(25)
            t.left(a1)
            t.left(90)
            t.forward(25)

            # Flecha Fuerza (P)
            t.pencolor('gold')
            t.pendown()
            t.forward(40)
            t.left(90)
            t.forward(10)
            t.right(135)
            t.forward(15)
            t.right(180)
            t.forward(15)
            t.left(135)
            t.forward(20)
            t.left(135)
            t.forward(15)
            t.left(180)
            t.forward(15)
            t.right(135)
            t.forward(10)
            t.left(90)
            t.forward(40)

            #Traslado
            t.penup()
            t.forward(25)
            t.right(90)
            t.right(a1)

            # Cuerda
            t.penup()
            t.backward(25)
            t.right(90)
            t.forward(20)
            t.left(90)
            t.backward((l1 / 2) + 20)
            t.right(-a1)
            t.forward(10)
            t.left(180)
            t.forward(10)
            t.left(45)
            t.forward(10)
            t.right(90)
            t.forward(10)
            t.left(45)
            t.pendown()
            t.pencolor('violet')
            t.right(-a2)
            t.left(180)
            t.backward((l2 / 2) + 20)

            # Massa1
            t.pencolor('black')
            t.left(90)
            t.forward(27)
            t.backward(50)
            t.left(90)
            t.forward(50)
            t.right(90)
            t.forward(50)
            t.right(90)
            t.forward(50)
            t.right(90)
            t.forward(27)
            t.penup()
            t.right(90)
            t.forward(25)
            t.pendown()
            t.forward(1)
            t.backward(1)
            t.penup()
            t.forward(25)

            # Linea de ejes M1
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
                    
            # Letra X del eje
            t.penup()
            t.forward(5)
            t.pendown()
            t.right(45)
            t.forward(10)
            t.left(90)
            t.forward(10)
            t.left(180)
            t.forward(20)
            t.right(180)
            t.forward(10)
            t.right(90)
            t.forward(10)
            t.left(180)
            t.forward(20)
            t.left(45)
            t.penup()
            t.forward(5)
            t.pendown()

            # Linea de ejes M1
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.forward(25)
            t.left(90)
            t.forward(25)
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
                    
            # Letra Y del eje
            t.penup()
            t.forward(5)
            t.pendown()
            t.right(45)
            t.forward(20)
            t.left(180)
            t.forward(10)
            t.right(90)
            t.forward(10)
            t.right(180)
            t.forward(10)
            t.right(90)
            t.forward(10)
            t.left(45)
            t.penup()
            t.forward(5)
            t.pendown()                    
                    
            # Linea de ejes M1
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.forward(50)
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.left(180)
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(30)

            # Flecha Fuerza (N2)
            t.pencolor('blue')
            t.forward(25)
            t.pendown()
            t.forward(40)
            t.left(90)
            t.forward(10)
            t.right(135)
            t.forward(15)
            t.right(180)
            t.forward(15)
            t.left(135)
            t.forward(20)
            t.left(135)
            t.forward(15)
            t.left(180)
            t.forward(15)
            t.right(135)
            t.forward(10)
            t.left(90)
            t.forward(40)

            #Traslado2
            t.penup()
            t.forward(25)
            t.left(90)
            t.forward(25)

            # Flecha Fuerza (Fr2)
            t.pencolor('red')
            t.pendown()
            t.forward(40)
            t.left(90)
            t.forward(10)
            t.right(135)
            t.forward(15)
            t.right(180)
            t.forward(15)
            t.left(135)
            t.forward(20)
            t.left(135)
            t.forward(15)
            t.left(180)
            t.forward(15)
            t.right(135)
            t.forward(10)
            t.left(90)
            t.forward(40)

            #Traslado2
            t.penup()
            t.forward(25)
            t.left(-a2)
            t.left(90)
            t.forward(25)

            # Flecha Fuerza (P2)
            t.pencolor('gold')
            t.pendown()
            t.forward(40)
            t.left(90)
            t.forward(10)
            t.right(135)
            t.forward(15)
            t.right(180)
            t.forward(15)
            t.left(135)
            t.forward(20)
            t.left(135)
            t.forward(15)
            t.left(180)
            t.forward(15)
            t.right(135)
            t.forward(10)
            t.left(90)
            t.forward(40)

            #Traslado2
            t.penup()
            t.forward(25)
            t.right(90)
            t.right(-a2)
            t.left(180)

            # Polea
            t.backward(25)
            t.backward((l2 / 2) + 20)
            t.right(a2)
            t.right(180)
            t.right(45)
            t.forward(10)
            t.right(135)
            t.forward(10)
            t.right(90)
            t.pendown()
            t.pencolor('black')
            t.circle(-10)


            # Cuadro de leyendass
            t.pensize(3)
            t.pencolor('black')

            t.penup()
            t.setpos(-150, 25)
            t.pendown()

            # Quadrado
            t.settiltangle(0)
            t.forward(50)

            # Primera linea divisoria
            t.left(90)
            t.forward(30)

            # Letra F
            t.pencolor('lightgreen')
            t.right(90)
            t.forward(30)
            t.right(90)
            t.forward(15)
            t.right(180)
            t.forward(15)
            t.left(90)
            t.forward(10)
            t.left(90)
            t.forward(10)
            t.left(180)
            t.forward(10)
            t.left(90)
            t.forward(20)

            # Primera linea divisoria
            t.pencolor('black')
            t.left(270)
            t.forward(55)

            # Letra F de Fr
            t.pencolor('red')
            t.right(90)
            t.forward(30)
            t.right(90)
            t.forward(15)
            t.right(180)
            t.forward(15)
            t.left(90)
            t.forward(10)
            t.left(90)
            t.forward(10)
            t.left(180)
            t.forward(10)
            t.left(90)
            t.forward(20)

            # Primera linea divisoria
            t.pencolor('black')
            t.left(270)
            t.forward(15)
            t.left(180)
            t.forward(25)

            # Letra R de Fr
            t.pencolor('red')
            t.left(90)
            t.forward(16)
            t.left(180)
            t.forward(4)
            t.left(90)
            t.forward(7)
            t.right(90)
            t.forward(3)
            t.left(180)
            t.forward(3)
            t.left(90)
            t.forward(7)
            t.left(90)
            t.forward(12)

            # Primera linea divisoria
            t.pencolor('black')
            t.left(90)
            t.forward(75)

            # Quadrado
            t.left(90)
            t.forward(50)
            t.left(90)
            t.forward(100)
            t.left(90)
            t.forward(100)
            t.left(90)
            t.forward(15)

            # Letra N
            t.pencolor('blue')
            t.left(90)
            t.forward(30)
            t.left(215)
            t.forward(37)
            t.right(215)
            t.forward(30)
            t.right(180)
            t.forward(30)
            t.left(215)
            t.forward(37)
            t.right(215)
            t.forward(30)

            # Quadrado
            t.pencolor('black')
            t.left(90)
            t.forward(35)

            # Segunda linea divisoria
            t.left(90)
            t.forward(100)
            t.left(180)
            t.forward(100)

            # Quadrado
            t.left(90)
            t.forward(20)

            # Letra P
            t.pencolor('Gold')
            t.left(90)
            t.forward(30)
            t.right(90)
            t.forward(15)
            t.right(90)
            t.forward(15)
            t.right(90)
            t.forward(15)
            t.left(90)
            t.forward(15)

            # Quadrado
            t.pencolor('black')
            t.left(90)
            t.forward(30)
            t.left(90)
            t.forward(150)
            t.left(90)
            t.forward(100)
            t.left(90)
            t.forward(50)
            t.left(90)
            t.forward(50)

            # Letra T
            t.left(90)
            t.forward(2)
            t.pencolor('violet')
            t.forward(35)
            t.left(90)
            t.forward(20)
            t.left(180)
            t.forward(40)

            # cual es cada massa
            t.pencolor('black')
            x = 170
            y = 160
            t.penup()
            t.goto(x, y)
            t.write(texto1, align="center", font=('Courier New', 10))

            x = -80
            y = 160
            t.goto(x, y)
            t.write(texto2, align="center", font=('Courier New', 10))

            # Esconder el lapiz
            t.penup()
            t.setpos(-320, -280)
            t.pendown()
            t.forward(1)
            t.hideturtle()
                
        elif xy1 >= 90:
            
            t.speed(100)

            a1 = float(angulo_de_inclinacion1.get())
            a2 = float(angulo_de_inclinacion2.get())
            a3 = 360 - (a1 + a2)

            l1 = 300
            l2 = (math.sin(math.radians(a1)) * 300) / math.sin(math.radians(a2))
            l3 = (math.sin(math.radians(a3)) * 300) / math.sin(math.radians(a2))

            #Triangulo de dos massas
            t.pensize(4)
            t.pencolor('grey')

            t.penup()
            t.setpos(210, -55)
            t.pendown()

            t.left(-a1)
            t.backward(l1)
            t.right(a3)
            t.backward(l2)
            t.right(a2)
            t.backward(l3)

            # Polea
            t.left(-a1)
            t.left(180)
            t.forward(l1)
            t.left(a1)
            t.pencolor('black')
            t.left(270)
            t.forward(10)
            t.penup()
            t.forward(10)
            t.pendown()
            t.forward(1)
            t.backward(1)
            t.left(45)
            t.penup()
            t.forward(10)
            t.pendown()
            t.left(90)
            t.circle(10, 270)

            # Cuerda
            t.pencolor('violet')
            t.right(45)
            t.penup()
            t.forward(10)
            t.left(180)
            t.forward(10)
            t.pendown()
            t.left(-90)
            t.right(a1)
            t.backward((l1 / 2) + 20)

            # Massa2
            t.pencolor('black')
            t.left(90)
            t.forward(25)
            t.backward(50)
            t.left(90)
            t.forward(50)
            t.right(90)
            t.forward(50)
            t.right(90)
            t.forward(50)
            t.right(90)
            t.forward(25)
            t.penup()
            t.right(90)
            t.forward(25)
            t.pendown()
            t.forward(1)
            t.backward(1)
            t.penup()
            t.forward(25)

            # Linea de ejes M2
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
                    
            # Letra X del eje
            t.penup()
            t.forward(5)
            t.pendown()
            t.left(45)
            t.forward(10)
            t.left(90)
            t.forward(10)
            t.left(180)
            t.forward(20)
            t.right(180)
            t.forward(10)
            t.right(90)
            t.forward(10)
            t.left(180)
            t.forward(20)
            t.right(45)
            t.penup()
            t.forward(5)
            t.pendown()

            # Linea de ejes M2
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.forward(25)
            t.left(90)
            t.forward(25)
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.left(180)
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.forward(50)
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
                    
            # Letra Y del eje
            t.penup()
            t.forward(5)
            t.pendown()
            t.right(45)
            t.forward(20)
            t.left(180)
            t.forward(10)
            t.right(90)
            t.forward(10)
            t.right(180)
            t.forward(10)
            t.right(90)
            t.forward(10)
            t.left(45)
            t.penup()
            t.forward(5)
            t.pendown()                    
                    
            # Linea de ejes M2
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(30)

            # Flecha Fuerza (N)
            t.pencolor('blue')
            t.left(180)
            t.forward(25)
            t.pendown()
            t.forward(40)
            t.left(90)
            t.forward(10)
            t.right(135)
            t.forward(15)
            t.right(180)
            t.forward(15)
            t.left(135)
            t.forward(20)
            t.left(135)
            t.forward(15)
            t.left(180)
            t.forward(15)
            t.right(135)
            t.forward(10)
            t.left(90)
            t.forward(40)

            #Traslado
            t.penup()
            t.forward(25)
            t.left(90)
            t.forward(25)

            # Flecha Fuerza (Fr)
            t.pencolor('red')
            t.pendown()
            t.forward(40)
            t.left(90)
            t.forward(10)
            t.right(135)
            t.forward(15)
            t.right(180)
            t.forward(15)
            t.left(135)
            t.forward(20)
            t.left(135)
            t.forward(15)
            t.left(180)
            t.forward(15)
            t.right(135)
            t.forward(10)
            t.left(90)
            t.forward(40)

            #Traslado
            t.penup()
            t.forward(25)
            t.left(a1)
            t.left(90)
            t.forward(25)

            # Flecha Fuerza (P)
            t.pencolor('gold')
            t.pendown()
            t.forward(40)
            t.left(90)
            t.forward(10)
            t.right(135)
            t.forward(15)
            t.right(180)
            t.forward(15)
            t.left(135)
            t.forward(20)
            t.left(135)
            t.forward(15)
            t.left(180)
            t.forward(15)
            t.right(135)
            t.forward(10)
            t.left(90)
            t.forward(40)

            #Traslado
            t.penup()
            t.forward(25)
            t.right(90)
            t.right(a1)

            # Cuerda
            t.penup()
            t.backward(25)
            t.backward((l1 / 2) + 20)
            t.right(-a1)
            t.forward(10)
            t.left(180)
            t.forward(10)
            t.left(45)
            t.forward(10)
            t.right(90)
            t.forward(10)
            t.left(45)
            t.pendown()
            t.pencolor('violet')
            t.right(-a2)
            t.left(180)
            t.backward((l2 / 2) + 20)

            # Massa1
            t.pencolor('black')
            t.left(90)
            t.forward(45)
            t.backward(50)
            t.left(90)
            t.forward(50)
            t.right(90)
            t.forward(50)
            t.right(90)
            t.forward(50)
            t.right(90)
            t.forward(25)
            t.penup()
            t.right(90)
            t.forward(25)
            t.pendown()
            t.forward(1)
            t.backward(1)
            t.penup()
            t.forward(25)

            # Linea de ejes M1
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
                    
            # Letra Y del eje
            t.penup()
            t.forward(5)
            t.pendown()
            t.left(45)
            t.forward(10)
            t.left(90)
            t.forward(10)
            t.left(180)
            t.forward(20)
            t.right(180)
            t.forward(10)
            t.left(90)
            t.forward(10)
            t.right(45)
            t.penup()
            t.forward(5)
            t.pendown()
                    
            # Linea de ejes M1
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.forward(25)
            t.left(90)
            t.forward(25)
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
                    
            # Letra X del eje
            t.penup()
            t.forward(5)
            t.pendown()
            t.right(45)
            t.forward(10)
            t.left(90)
            t.forward(10)
            t.left(180)
            t.forward(20)
            t.right(180)
            t.forward(10)
            t.right(90)
            t.forward(10)
            t.left(180)
            t.forward(20)
            t.left(45)
            t.penup()
            t.forward(5)
            t.pendown()
                    
            # Linea de ejes M1
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.forward(50)
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.left(180)
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(30)

            # Flecha Fuerza (N2)
            t.pencolor('blue')
            t.forward(25)
            t.pendown()
            t.forward(40)
            t.left(90)
            t.forward(10)
            t.right(135)
            t.forward(15)
            t.right(180)
            t.forward(15)
            t.left(135)
            t.forward(20)
            t.left(135)
            t.forward(15)
            t.left(180)
            t.forward(15)
            t.right(135)
            t.forward(10)
            t.left(90)
            t.forward(40)

            #Traslado2
            t.penup()
            t.forward(25)
            t.left(90)
            t.forward(25)

            # Flecha Fuerza (Fr2)
            t.pencolor('red')
            t.pendown()
            t.forward(40)
            t.left(90)
            t.forward(10)
            t.right(135)
            t.forward(15)
            t.right(180)
            t.forward(15)
            t.left(135)
            t.forward(20)
            t.left(135)
            t.forward(15)
            t.left(180)
            t.forward(15)
            t.right(135)
            t.forward(10)
            t.left(90)
            t.forward(40)

            #Traslado2
            t.penup()
            t.forward(25)
            t.left(-a2)
            t.left(90)
            t.forward(25)

            # Flecha Fuerza (P2)
            t.pencolor('gold')
            t.pendown()
            t.forward(40)
            t.left(90)
            t.forward(10)
            t.right(135)
            t.forward(15)
            t.right(180)
            t.forward(15)
            t.left(135)
            t.forward(20)
            t.left(135)
            t.forward(15)
            t.left(180)
            t.forward(15)
            t.right(135)
            t.forward(10)
            t.left(90)
            t.forward(40)

            #Traslado2
            t.penup()
            t.forward(25)
            t.right(90)
            t.right(-a2)
            t.left(180)

            # Polea
            t.backward(25)
            t.left(90)
            t.forward(20)
            t.right(90)
            t.backward((l2 / 2) + 20)
            t.right(a2)
            t.right(180)
            t.right(45)
            t.forward(10)
            t.right(135)
            t.forward(10)
            t.right(90)
            t.pendown()
            t.pencolor('black')
            t.circle(-10)


            # Cuadro de leyendass
            t.pensize(3)
            t.pencolor('black')

            t.penup()
            t.setpos(-150, 25)
            t.pendown()

            # Quadrado
            t.settiltangle(0)
            t.forward(50)

            # Primera linea divisoria
            t.left(90)
            t.forward(30)

            # Letra F
            t.pencolor('lightgreen')
            t.right(90)
            t.forward(30)
            t.right(90)
            t.forward(15)
            t.right(180)
            t.forward(15)
            t.left(90)
            t.forward(10)
            t.left(90)
            t.forward(10)
            t.left(180)
            t.forward(10)
            t.left(90)
            t.forward(20)

            # Primera linea divisoria
            t.pencolor('black')
            t.left(270)
            t.forward(55)

            # Letra F de Fr
            t.pencolor('red')
            t.right(90)
            t.forward(30)
            t.right(90)
            t.forward(15)
            t.right(180)
            t.forward(15)
            t.left(90)
            t.forward(10)
            t.left(90)
            t.forward(10)
            t.left(180)
            t.forward(10)
            t.left(90)
            t.forward(20)

            # Primera linea divisoria
            t.pencolor('black')
            t.left(270)
            t.forward(15)
            t.left(180)
            t.forward(25)

            # Letra R de Fr
            t.pencolor('red')
            t.left(90)
            t.forward(16)
            t.left(180)
            t.forward(4)
            t.left(90)
            t.forward(7)
            t.right(90)
            t.forward(3)
            t.left(180)
            t.forward(3)
            t.left(90)
            t.forward(7)
            t.left(90)
            t.forward(12)

            # Primera linea divisoria
            t.pencolor('black')
            t.left(90)
            t.forward(75)

            # Quadrado
            t.left(90)
            t.forward(50)
            t.left(90)
            t.forward(100)
            t.left(90)
            t.forward(100)
            t.left(90)
            t.forward(15)

            # Letra N
            t.pencolor('blue')
            t.left(90)
            t.forward(30)
            t.left(215)
            t.forward(37)
            t.right(215)
            t.forward(30)
            t.right(180)
            t.forward(30)
            t.left(215)
            t.forward(37)
            t.right(215)
            t.forward(30)

            # Quadrado
            t.pencolor('black')
            t.left(90)
            t.forward(35)

            # Segunda linea divisoria
            t.left(90)
            t.forward(100)
            t.left(180)
            t.forward(100)

            # Quadrado
            t.left(90)
            t.forward(20)

            # Letra P
            t.pencolor('Gold')
            t.left(90)
            t.forward(30)
            t.right(90)
            t.forward(15)
            t.right(90)
            t.forward(15)
            t.right(90)
            t.forward(15)
            t.left(90)
            t.forward(15)

            # Quadrado
            t.pencolor('black')
            t.left(90)
            t.forward(30)
            t.left(90)
            t.forward(150)
            t.left(90)
            t.forward(100)
            t.left(90)
            t.forward(50)
            t.left(90)
            t.forward(50)

            # Letra T
            t.left(90)
            t.forward(2)
            t.pencolor('violet')
            t.forward(35)
            t.left(90)
            t.forward(20)
            t.left(180)
            t.forward(40)

            # cual es cada massa
            t.pencolor('black')
            x = 170
            y = 160
            t.penup()
            t.goto(x, y)
            t.write(texto1, align="center", font=('Courier New', 10))

            x = -80
            y = 160
            t.goto(x, y)
            t.write(texto2, align="center", font=('Courier New', 10))

            # Esconder el lapiz
            t.penup()
            t.setpos(-320, -280)
            t.pendown()
            t.forward(1)
            t.hideturtle()

        else:

            t.speed(100)

            a1 = float(angulo_de_inclinacion1.get())
            a2 = float(angulo_de_inclinacion2.get())
            a3 = 360 - (a1 + a2)

            l1 = (math.sin(math.radians(a2)) * 350) / math.sin(math.radians(a3))
            l2 = (math.sin(math.radians(a1)) * 350) / math.sin(math.radians(a3))
            l3 = 350

            #Triangulo de dos massas
            t.pensize(4)
            t.pencolor('grey')

            t.penup()
            t.setpos(220, -120)
            t.pendown()

            t.left(-a1)
            t.forward(l1)
            t.right(a3)
            t.forward(l2)
            t.right(a2)
            t.forward(l3)

            # Polea
            t.left(-a1)
            t.forward(l1)
            t.left(a1)
            t.pencolor('black')
            t.left(90)
            t.forward(10)
            t.penup()
            t.forward(10)
            t.pendown()
            t.forward(1)
            t.backward(1)
            t.left(45)
            t.penup()
            t.forward(10)
            t.pendown()
            t.left(90)
            t.circle(10, 270)

            # Cuerda
            t.pencolor('violet')
            t.right(45)
            t.penup()
            t.forward(10)
            t.left(180)
            t.forward(10)
            t.pendown()
            t.left(-90)
            t.right(a1)
            t.forward((l1 / 2) + 20)

            # Massa2
            t.pencolor('black')
            t.left(90)
            t.forward(25)
            t.backward(50)
            t.left(90)
            t.forward(50)
            t.right(90)
            t.forward(50)
            t.right(90)
            t.forward(50)
            t.right(90)
            t.forward(25)
            t.penup()
            t.right(90)
            t.forward(25)
            t.pendown()
            t.forward(1)
            t.backward(1)
            t.penup()
            t.forward(25)

            # Linea de ejes M2
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
                    
            # Letra X del eje
            t.penup()
            t.forward(5)
            t.pendown()
            t.left(45)
            t.forward(10)
            t.left(90)
            t.forward(10)
            t.left(180)
            t.forward(20)
            t.right(180)
            t.forward(10)
            t.right(90)
            t.forward(10)
            t.left(180)
            t.forward(20)
            t.right(45)
            t.penup()
            t.forward(5)
            t.pendown()

            # Linea de ejes M1
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.forward(25)
            t.left(90)
            t.forward(25)
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.left(180)
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.forward(50)
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
                    
            # Letra Y del eje
            t.penup()
            t.forward(5)
            t.pendown()
            t.right(45)
            t.forward(20)
            t.left(180)
            t.forward(10)
            t.right(90)
            t.forward(10)
            t.right(180)
            t.forward(10)
            t.right(90)
            t.forward(10)
            t.left(45)
            t.penup()
            t.forward(5)
            t.pendown()                    
                    
            # Linea de ejes M1
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(30)

            # Flecha Fuerza (N)
            t.pencolor('blue')
            t.left(180)
            t.forward(25)
            t.pendown()
            t.forward(40)
            t.left(90)
            t.forward(10)
            t.right(135)
            t.forward(15)
            t.right(180)
            t.forward(15)
            t.left(135)
            t.forward(20)
            t.left(135)
            t.forward(15)
            t.left(180)
            t.forward(15)
            t.right(135)
            t.forward(10)
            t.left(90)
            t.forward(40)

            #Traslado
            t.penup()
            t.forward(25)
            t.left(90)
            t.forward(25)

            # Flecha Fuerza (Fr)
            t.pencolor('red')
            t.pendown()
            t.forward(40)
            t.left(90)
            t.forward(10)
            t.right(135)
            t.forward(15)
            t.right(180)
            t.forward(15)
            t.left(135)
            t.forward(20)
            t.left(135)
            t.forward(15)
            t.left(180)
            t.forward(15)
            t.right(135)
            t.forward(10)
            t.left(90)
            t.forward(40)

            #Traslado
            t.penup()
            t.forward(25)
            t.left(a1)
            t.left(90)
            t.forward(25)

            # Flecha Fuerza (P)
            t.pencolor('gold')
            t.pendown()
            t.forward(40)
            t.left(90)
            t.forward(10)
            t.right(135)
            t.forward(15)
            t.right(180)
            t.forward(15)
            t.left(135)
            t.forward(20)
            t.left(135)
            t.forward(15)
            t.left(180)
            t.forward(15)
            t.right(135)
            t.forward(10)
            t.left(90)
            t.forward(40)

            #Traslado
            t.penup()
            t.forward(25)
            t.right(90)
            t.right(a1)

            # Cuerda
            t.penup()
            t.backward(25)
            t.forward((l1 / 2) + 20)
            t.right(-a1)
            t.forward(10)
            t.left(180)
            t.forward(10)
            t.left(45)
            t.forward(10)
            t.right(90)
            t.forward(10)
            t.left(45)
            t.pendown()
            t.pencolor('violet')
            t.right(-a2)
            t.left(180)
            t.forward((l2 / 2) + 20)

            # Massa1
            t.pencolor('black')
            t.left(90)
            t.forward(27)
            t.backward(50)
            t.left(90)
            t.forward(50)
            t.right(90)
            t.forward(50)
            t.right(90)
            t.forward(50)
            t.right(90)
            t.forward(27)
            t.penup()
            t.right(90)
            t.forward(25)
            t.pendown()
            t.forward(1)
            t.backward(1)
            t.penup()
            t.forward(25)

            # Linea de ejes M1
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
                    
            # Letra X del eje
            t.penup()
            t.forward(5)
            t.pendown()
            t.right(45)
            t.forward(10)
            t.left(90)
            t.forward(10)
            t.left(180)
            t.forward(20)
            t.right(180)
            t.forward(10)
            t.right(90)
            t.forward(10)
            t.left(180)
            t.forward(20)
            t.left(45)
            t.penup()
            t.forward(5)
            t.pendown()

            # Linea de ejes M1
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.forward(25)
            t.left(90)
            t.forward(25)
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
                    
            # Letra Y del eje
            t.penup()
            t.forward(5)
            t.pendown()
            t.right(45)
            t.forward(20)
            t.left(180)
            t.forward(10)
            t.right(90)
            t.forward(10)
            t.right(180)
            t.forward(10)
            t.right(90)
            t.forward(10)
            t.left(45)
            t.penup()
            t.forward(5)
            t.pendown()                    
                    
            # Linea de ejes M1
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.forward(50)
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.left(180)
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(5)
            t.pendown()
            t.forward(10)
            t.penup()
            t.forward(30)

            # Flecha Fuerza (N2)
            t.pencolor('blue')
            t.forward(25)
            t.pendown()
            t.forward(40)
            t.left(90)
            t.forward(10)
            t.right(135)
            t.forward(15)
            t.right(180)
            t.forward(15)
            t.left(135)
            t.forward(20)
            t.left(135)
            t.forward(15)
            t.left(180)
            t.forward(15)
            t.right(135)
            t.forward(10)
            t.left(90)
            t.forward(40)

            #Traslado2
            t.penup()
            t.forward(25)
            t.left(90)
            t.forward(25)

            # Flecha Fuerza (Fr2)
            t.pencolor('red')
            t.pendown()
            t.forward(40)
            t.left(90)
            t.forward(10)
            t.right(135)
            t.forward(15)
            t.right(180)
            t.forward(15)
            t.left(135)
            t.forward(20)
            t.left(135)
            t.forward(15)
            t.left(180)
            t.forward(15)
            t.right(135)
            t.forward(10)
            t.left(90)
            t.forward(40)

            #Traslado2
            t.penup()
            t.forward(25)
            t.left(-a2)
            t.left(90)
            t.forward(25)

            # Flecha Fuerza (P2)
            t.pencolor('gold')
            t.pendown()
            t.forward(40)
            t.left(90)
            t.forward(10)
            t.right(135)
            t.forward(15)
            t.right(180)
            t.forward(15)
            t.left(135)
            t.forward(20)
            t.left(135)
            t.forward(15)
            t.left(180)
            t.forward(15)
            t.right(135)
            t.forward(10)
            t.left(90)
            t.forward(40)

            #Traslado2
            t.penup()
            t.forward(25)
            t.right(90)
            t.right(-a2)
            t.left(180)

            # Polea
            t.backward(25)
            t.forward((l2 / 2) + 20)
            t.right(a2)
            t.right(180)
            t.right(45)
            t.forward(10)
            t.right(135)
            t.forward(10)
            t.right(90)
            t.pendown()
            t.pencolor('black')
            t.circle(-10)


            # Cuadro de leyendass
            t.pensize(3)
            t.pencolor('black')

            t.penup()
            t.setpos(-150, 25)
            t.pendown()

            # Quadrado
            t.settiltangle(0)
            t.forward(50)

            # Primera linea divisoria
            t.left(90)
            t.forward(30)

            # Letra F
            t.pencolor('lightgreen')
            t.right(90)
            t.forward(30)
            t.right(90)
            t.forward(15)
            t.right(180)
            t.forward(15)
            t.left(90)
            t.forward(10)
            t.left(90)
            t.forward(10)
            t.left(180)
            t.forward(10)
            t.left(90)
            t.forward(20)

            # Primera linea divisoria
            t.pencolor('black')
            t.left(270)
            t.forward(55)

            # Letra F de Fr
            t.pencolor('red')
            t.right(90)
            t.forward(30)
            t.right(90)
            t.forward(15)
            t.right(180)
            t.forward(15)
            t.left(90)
            t.forward(10)
            t.left(90)
            t.forward(10)
            t.left(180)
            t.forward(10)
            t.left(90)
            t.forward(20)

            # Primera linea divisoria
            t.pencolor('black')
            t.left(270)
            t.forward(15)
            t.left(180)
            t.forward(25)

            # Letra R de Fr
            t.pencolor('red')
            t.left(90)
            t.forward(16)
            t.left(180)
            t.forward(4)
            t.left(90)
            t.forward(7)
            t.right(90)
            t.forward(3)
            t.left(180)
            t.forward(3)
            t.left(90)
            t.forward(7)
            t.left(90)
            t.forward(12)

            # Primera linea divisoria
            t.pencolor('black')
            t.left(90)
            t.forward(75)

            # Quadrado
            t.left(90)
            t.forward(50)
            t.left(90)
            t.forward(100)
            t.left(90)
            t.forward(100)
            t.left(90)
            t.forward(15)

            # Letra N
            t.pencolor('blue')
            t.left(90)
            t.forward(30)
            t.left(215)
            t.forward(37)
            t.right(215)
            t.forward(30)
            t.right(180)
            t.forward(30)
            t.left(215)
            t.forward(37)
            t.right(215)
            t.forward(30)

            # Quadrado
            t.pencolor('black')
            t.left(90)
            t.forward(35)

            # Segunda linea divisoria
            t.left(90)
            t.forward(100)
            t.left(180)
            t.forward(100)

            # Quadrado
            t.left(90)
            t.forward(20)

            # Letra P
            t.pencolor('Gold')
            t.left(90)
            t.forward(30)
            t.right(90)
            t.forward(15)
            t.right(90)
            t.forward(15)
            t.right(90)
            t.forward(15)
            t.left(90)
            t.forward(15)

            # Quadrado
            t.pencolor('black')
            t.left(90)
            t.forward(30)
            t.left(90)
            t.forward(150)
            t.left(90)
            t.forward(100)
            t.left(90)
            t.forward(50)
            t.left(90)
            t.forward(50)

            # Letra T
            t.left(90)
            t.forward(2)
            t.pencolor('violet')
            t.forward(35)
            t.left(90)
            t.forward(20)
            t.left(180)
            t.forward(40)

            # cual es cada massa
            t.pencolor('black')
            x = 170
            y = 130
            t.penup()
            t.goto(x, y)
            t.write(texto1, align="center", font=('Courier New', 10))

            x = -80
            y = 130
            t.goto(x, y)
            t.write(texto2, align="center", font=('Courier New', 10))

            # Esconder el lapiz
            t.penup()
            t.setpos(-320, -280)
            t.pendown()
            t.forward(1)
            t.hideturtle()
    
    # Mantiene la ventana simpre abierta hasta que el usuario la cierra
    ventana2.mainloop()
