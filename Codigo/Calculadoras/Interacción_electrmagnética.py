...
def electromagnetismo():
    ...
    etiqueta_posición_inicial_partícula.grid(row=0, column=4, padx=10, pady=10)
    etiqueta_posición_campo_magnético = ctk.CTkLabel(ventana11, text='Posición del campo magnético', text_color='white')
    etiqueta_posición_campo_magnético.grid(row=1, column=0, padx=10, pady=10)
    etiqueta_intensidad_campo_magnético = ctk.CTkLabel(ventana11, text='Intensidad del Campo magnético (T)', text_color='white')
    etiqueta_intensidad_campo_magnético.grid(row=1, column=2, padx=10, pady=10)
    ...
    posición_P_X = ctk.CTkEntry(ventana11)
    posición_P_X.grid(row=0, column=5, padx=10, pady=10)
    posición_P_Y = ctk.CTkEntry(ventana11)
    posición_P_Y.grid(row=1, column=1, padx=10, pady=10)
    posición_P_Z = ctk.CTkEntry(ventana11)
    posición_P_Z.grid(row=1, column=3, padx=10, pady=10)
        # Posición Campo Magnético
    posición_CM_X = ctk.CTkEntry(ventana11)
    posición_CM_X.grid(row=2, column=1, padx=10, pady=10)
    posición_CM_Y = ctk.CTkEntry(ventana11)
    posición_CM_Y.grid(row=2, column=3, padx=10, pady=10)
    posición_CM_Z = ctk.CTkEntry(ventana11)
    posición_CM_Z.grid(row=2, column=5, padx=10, pady=10)
    # Entadas de datos
    intensidad_CM = ctk.CTkEntry(ventana11)
    intensidad_CM.grid(row=3, column=3, padx=10, pady=10)
    velocidad_IP = ctk.CTkEntry(ventana11)
    velocidad_IP.grid(row=3, column=5, padx=10, pady=10)
